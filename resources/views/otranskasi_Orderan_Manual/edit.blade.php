	...
	<div class="content-wrapper">
		<div class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0">{{ $status == 'simpan' ? 'New' : 'Edit' }} Orderan Manual</h1>
					</div>
				</div>
			</div>
		</div>
						...
						<div class="card-header">
							<div class="row">
								<div class="col-md-6">
									...
									<a href="{{ route('torderanmanual') }}" class="btn btn-sm btn-danger">
												...
												<tr data-no-id="{{ $row->no_id ?? 0 }}">
													...
													<td>
														<input type="text" class="form-control form-control-sm stok text-right" name="detail[{{ $key }}][stok]"
															value="{{ number_format($row->stok ?? 0, 2, '.', '') }}" readonly style="background-color: #e9ecef;">
													</td>
	...
	<script>
		$(document).ready(function() {
			...
			function searchBarang(q) {
				...
				$.ajax({
					url: "{{ route('torderanmanual.browse') }}",
					data: {
						q: q
					},
			...
			function processBarang() {
				...
				$.ajax({
					url: "{{ route('torderanmanual.detail') }}",
					data: {
						kd_brg: kdBrg
					},
			...
			$(document).on('click', '.btn-delete-row', function() {
				...
				if ($('#tbody-detail tr').length === 0) {
					$('#tbody-detail').html('<tr><td colspan="7" class="text-center">Tidak ada data</td></tr>');
				}
			...
			$('#btn-save').click(function(e) {
				...
				Swal.fire({
					...
					preConfirm: () => {
						...
						return $.ajax({
							url: "{{ route('torderanmanual.store') }}",
							type: 'POST',
							data: formData,
							dataType: 'json'
						});
					...
					if (result.isConfirmed) {
						Swal.fire({
							...
							window.location.href = "{{ route('torderanmanual') }}";
						});
					}
				}).catch((error) => {
