	...
	<div class="content-wrapper">
		<div class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0">Orderan Manual</h1>
					</div>
					<div class="col-sm-6">
						<ol class="breadcrumb float-sm-right">
							<li class="breadcrumb-item"><a href="/">Home</a></li>
							<li class="breadcrumb-item active">Orderan Manual</li>
						</ol>
					</div>
				</div>
			</div>
					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<div class="row">
									<div class="col-md-6">
										<a href="{{ route('torderanmanual.edit', ['status' => 'simpan']) }}" class="btn btn-primary">
	...
	<script>
		$(document).ready(function() {
			...
			var table = $('#datatable').DataTable({
				...
				pageLength: 25,
				ajax: {
					url: "{{ route('torderanmanual.get-data') }}",
			...
			$(document).on('click', '[data-action="print"]', function(e) {
				...
				$.ajax({
					url: "{{ route('torderanmanual.print') }}",
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						no_bukti: noBukti
					...
					success: function(response) {
						...
						if (response.data && response.data.length > 0) {
							...
							printWindow.document.write('<html><head><title>Print Orderan Manual</title>');
							...
							printWindow.document.write('<h2>Orderan Manual</h2>');
							printWindow.document.write('<p>No. Bukti: ' + noBukti + '</p>');
							printWindow.document.write('<p>Tanggal: ' + (response.data[0].tgl || '') + '</p>');
							printWindow.document.write(
								'<table><thead><tr><th class="text-center">No</th><th>Sub</th><th>Kode Bar</th><th>Nama Barang</th><th>Kemasan</th><th class="text-right">Stock GD</th><th class="text-right">Stock TK</th><th class="text-right">KD</th><th class="text-right">SMAX TK</th><th class="text-right">DTR</th><th class="text-right">Qty</th><th>Ket</th></tr></thead><tbody>'
							);
		...
		function editData(noBukti) {
			window.location.href = "{{ route('torderanmanual.edit') }}?status=edit&no_bukti=" + noBukti;
		}
		...
		function deleteData(noBukti) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "{{ url('torderanmanual') }}/" + noBukti,
						type: 'DELETE',
						data: {
							_token: '{{ csrf_token() }}'
						},
