					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<h3 class="card-title">Stok Opname Koreksi Hadiah - {{ $status == 'simpan' ? 'New' : 'Edit' }}</h3>
							</div>
							<div class="card-body">
								<form action="{{ route('phstokopnamekoreksihadiah.store') }}" method="POST" name="entri" id="entri">
									@csrf
									<input type="hidden" id="status" name="status" value="{{ $status }}">
									<input type="hidden" id="flag" name="flag" value="HZ">
											...
											<div class="row">
												<div class="col-md-6">
													<div class="form-group row">
														...
														<div class="col-md-9">
															<input type="text" class="form-control form-control-sm" id="type" name="type" value="{{ $header->TYPE ?? '' }}"
																placeholder="Type">
														</div>
													</div>
												</div>
												...
												<div class="col-md-6">
													<div class="form-group row">
														<div class="col-md-3">
															<label for="bktk" class="form-label">BKTK</label>
														</div>
														<div class="col-md-9">
															<input type="text" class="form-control form-control-sm" id="bktk" name="bktk" value="{{ $header->BKTK ?? '' }}"
																placeholder="BKTK">
														</div>
													</div>
												</div>
											...
											<div class="row">
												<div class="col-md-12">
													<div class="form-group row">
														...
														<div class="col-md-11">
															<textarea class="form-control form-control-sm" id="notes" name="notes" rows="2" placeholder="Notes">{{ $header->NOTES ?? '' }}</textarea>
														</div>
													</div>
												</div>
											</div>
												...
												<div class="card">
													...
													<div class="card-body p-2">
														...
														<div class="row">
															...
															<div class="col-md-2">
																<div class="form-group">
																	<label for="riil_entry" class="form-label">Riil</label>
																	<input type="number" step="0.01" class="form-control form-control-sm text-right" id="riil_entry" name="riil_entry"
																		placeholder="0.00">
																</div>
															</div>
															...
															<div class="col-md-2">
																<div class="form-group">
																	...
																	<input type="number" step="0.01" class="form-control form-control-sm text-right" id="qty_entry" name="qty_entry"
																...
																@if ($status == 'edit' && !empty($detail))
																	@foreach ($detail as $index => $item)
																		<tr data-kd-brg="{{ $item->kd_brg }}">
																			...
																			<td>{{ $item->kd_brg }}</td>
																			<td>{{ $item->na_brg }}</td>
																			<td class="text-right">{{ number_format($item->qty - $item->qty, 2, ',', '.') }}</td>
	...
	<script>
		...
		function handleKdBrgEnter(kd_brg) {
			if (kd_brg) {
				...
				$.ajax({
					url: '{{ route('phstokopnamekoreksihadiah.detail') }}',
					type: 'GET',
					data: {
						kd_brg: kd_brg,
						flag: flag
		...
		function calculateQtyEntry() {
			var riil = parseFloat($('#riil_entry').val()) || 0;
			var stok = TEMP_PRODUCT ? parseFloat(TEMP_PRODUCT.saldo) : 0;
			var qty = riil - stok;
			$('#qty_entry').val(qty.toFixed(2));
		}
		...
		function addToDetail() {
			...
			addNewRow(TEMP_PRODUCT, riil, qty, ket);
			clearEntryForm();
			calculateTotal();
			$('#kd_brg').focus();
		}

		function addNewRow(product, riil, qty, ket) {
			var stok = parseFloat(product.saldo);
			var newRow = `
		<tr data-kd-brg="${product.kd_brg}">
			<td class="text-center">${detailRowIndex + 1}</td>
			<td>${product.kd_brg}</td>
			<td>${product.na_brg}</td>
			<td class="text-right">${formatNumber(stok)}</td>
			<td class="text-right">${formatNumber(riil)}</td>
			<td class="text-right">${formatNumber(qty)}</td>
			<td><input type="text" class="form-control form-control-sm" name="details[${detailRowIndex}][ket]" value="${ket}"></td>
		...
		function searchProduct(query) {
			...
			$.ajax({
				url: '{{ route('phstokopnamekoreksihadiah.browse') }}',
				type: 'GET',
				data: {
					q: query,
					flag: flag
					...
					if (response.length > 0) {
						response.forEach(function(item) {
							...
							row += '<td><button type="button" class="btn btn-primary btn-sm" onclick="selectProduct(\'' +
								item.kd_brg + '\', \'' + escapeHtml(item.na_brg) + '\', ' + item.saldo + ')">Select</button></td>';
							row += '</tr>';
							tbody.append(row);
						});
					} else {
		...
		function simpan() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						url: '{{ route('phstokopnamekoreksihadiah.store') }}',
						type: 'POST',
						...
						success: function(response) {
							...
							if (response.success) {
								Swal.fire({
									...
									window.location.href = '{{ route('phstokopnamekoreksihadiah') }}';
								});
							} else {
								Swal.fire({
									title: 'Error!',
