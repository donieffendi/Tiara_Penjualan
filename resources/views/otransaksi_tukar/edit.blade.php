                ...
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/tukar/store?flagz='.$flagz.'') : url('/tukar/update/'.$header->NO_ID.'&flagz='.$flagz.'' ) }}" method="POST" name ="entri" id="entri" >
							...
							<div class="form-group row">
								...
								<div class="col-md-2">
									<select id="TYPE" class="form-control"  name="TYPE">
										...
										<option value="LAIN" {{ ($header->TYPE == 'LAIN') ? 'selected' : '' }}>LAIN</option>
										<option value="HBESAR" {{ ($header->TYPE == 'HBESAR') ? 'selected' : '' }}>HARI BESAR</option>
										<option value="PROMO" {{ ($header->TYPE == 'PROMO') ? 'selected' : '' }}>PROMO</option>
									</select>
                            	</div>
                            ...
                            <table id="datatable" class="table table-striped table-border">
								...
								@foreach ($detail as $detail)		
                                    <tr>
										...
										<td><input name="HARGA[]" onclick="select()" onblur="hitung()" value="{{$detail->HARGA}}" id="HARGA{{$no}}" type="text" style="text-align: right"  readonly class="form-control HARGA text-primary"></td>
										<td><input name="TOTAL[]" onclick="select()" onblur="hitung()" value="{{$detail->TOTAL}}" id="TOTAL{{$no}}" type="text" style="text-align: right"  readonly class="form-control TOTAL text-primary"></td>
								...
								<td></td>
                                    ...
                                    <td></td>
                                     <td><input class="form-control TTOTAL  text-primary font-weight-bold" style="text-align: right"  id="TTOTAL" name="TTOTAL_QTY" value="{{$header->TOTAL_QTY}}" readonly></td>
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button hidden type="button" id='TOPX'  onclick="location.href='{{url('/tukar/edit/?idx=' .$idx. '&tipx=top&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Top</button>
								<button hidden type="button" id='PREVX' onclick="location.href='{{url('/tukar/edit/?idx='.$header->NO_ID.'&tipx=prev&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Prev</button>
								<button hidden type="button" id='NEXTX' onclick="location.href='{{url('/tukar/edit/?idx='.$header->NO_ID.'&tipx=next&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Next</button>
								<button hidden type="button" id='BOTTOMX' onclick="location.href='{{url('/tukar/edit/?idx=' .$idx. '&tipx=bottom&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
								<button hidden type="button" id='NEWX' onclick="location.href='{{url('/tukar/edit/?idx=0&tipx=new&flagz='.$flagz.'' )}}'" class="btn btn-warning">New</button>
								<button hidden type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>                    
								<button hidden type="button" id='UNDOX' onclick="location.href='{{url('/tukar/edit/?idx=' .$idx. '&tipx=undo&flagz='.$flagz.'' )}}'" class="btn btn-info">Undo</button>  
			...
			$.ajax(
			{
...

				{
					...
					{	
							...
							for(i=0; i<resp.length; i++){
								...
								dTableBBarang.row.add([
									'<a href="javascript:void(0);" onclick="chooseBarang(\''+resp[i].KD_BRG+'\', \''+resp[i].NA_BRG+'\' , \''+resp[i].SATUAN+'\'  , \''+resp[i].HJUAL+'\' )">'+resp[i].KD_BRG+'</a>',
									resp[i].NA_BRG,
									resp[i].SATUAN,
									resp[i].HJUAL,
								]);
					...
					else
					{
						...
						$("#HARGA"+rowidBarang).val(resp[0].HJUAL);
					}
				}
			});
		}
