										...
										<div class="form-group row">
											...
											<div class="col-md-9">
												<select class="form-control form-control-sm" id="jns_dis" name="jns_dis" required>
													...
													<option value="CASHBACK" {{ ($header->jns_dis ?? '') == 'CASHBACK' ? 'selected' : '' }}>CASHBACK</option>
													<option value="DISKON" {{ ($header->jns_dis ?? '') == 'DISKON' ? 'selected' : '' }}>DISKON</option>
												</select>
											</div>
										</div>
...
<script>
	...
	function appendBarang(type, data) {
		...
		var na_brg = data.na_brgh || data.na_brg;

		label.text(kd_brg + ' - ' + na_brg);
