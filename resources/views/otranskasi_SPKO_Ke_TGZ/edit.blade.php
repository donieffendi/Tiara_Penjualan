	...
	<div class="content-wrapper">
		<div class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0">{{ $status == 'simpan' ? 'New' : 'Edit' }} SPKO ke TGZ</h1>
					</div>
				</div>
			</div>
		</div>
						...
						<div class="card-header">
							<div class="row">
								<div class="col-md-6">
									...
									<a href="{{ route('tspkoketgz') }}" class="btn btn-sm btn-danger">
									...
									<div class="form-group row">
										...
										<div class="col-sm-8">
											<select class="form-control form-control-sm" name="cbg" id="cbg" required {{ $posted == 1 ? 'disabled' : '' }}>
												...
												@foreach ($cabang as $c)
													@if (($c->KODE ?? ($c->kode ?? '')) != $ma)
														<option value="{{ $c->KODE ?? ($c->kode ?? '') }}" {{ ($header->CBG ?? $cbg) == ($c->KODE ?? ($c->kode ?? '')) ? 'selected' : '' }}>
															{{ $c->KODE ?? ($c->kode ?? '') }} - {{ $c->NA_TOKO ?? ($c->na_toko ?? ($c->nama ?? '')) }}
														</option>
													@endif
												@endforeach
											</select>
							...
							<div class="row mt-3">
								<div class="col-md-6">
									<div class="form-group row">
										...
										<div class="col-sm-6">
											<input type="text" class="form-control form-control-sm" id="kd_brg" placeholder="Kode Barang" {{ $posted == 1 ? 'readonly' : '' }}>
										</div>
										<div class="col-sm-3">
											@if ($posted != 1)
								...
								<div class="col-md-6">
									<div class="form-group row">
										...
										<div class="col-sm-4">
											<input type="number" class="form-control form-control-sm text-right" id="qty" step="0.01" {{ $posted == 1 ? 'readonly' : '' }}>
										</div>
										<div class="col-sm-6">
								...
								<table class="table-bordered table-striped table-sm table" id="table-detail">
									<thead class="thead-dark">
										<tr>
											...
											<th width="8%" class="text-center">Notes</th>
											@if ($posted != 1)
									...
									<tbody id="tbody-detail">
										...
										@else
											<tr>
												<td colspan="{{ $posted != 1 ? '9' : '8' }}" class="text-center">Tidak ada data</td>
											</tr>
										@endif
									</tbody>
								</table>
					...
					<div class="table-responsive">
						<table class="table-bordered table-sm table">
							...
							<tbody id="tbody-browse">
								<tr>
									<td colspan="4" class="text-center">Ketik untuk mencari...</td>
								</tr>
							</tbody>
						</table>
					</div>
	...
	<script>
		$(document).ready(function() {
			...
			$('#btn-browse').click(function() {
				...
				$('#search-barang').val('');
				$('#tbody-browse').html('<tr><td colspan="4" class="text-center">Ketik untuk mencari...</td></tr>');
			...
			$('#search-barang').on('keyup', function() {
				...
				if (q.length >= 2) {
					...
					$('#tbody-browse').html('<tr><td colspan="4" class="text-center">Ketik minimal 2 karakter...</td></tr>');
				}
			});
			...
			function searchBarang(q) {
				...
				$('#tbody-browse').html('<tr><td colspan="4" class="text-center"><i class="fas fa-spinner fa-spin"></i> Loading...</td></tr>');

				$.ajax({
					url: "{{ route('tspkoketgz.browse') }}",
					data: {
						type: 'barang',
						q: q,
						cbg: cbg
					...
					error: function(xhr) {
						$('#tbody-browse').html('<tr><td colspan="4" class="text-center text-danger">Error: ' + (xhr.responseJSON
					...
					success: function(response) {
						if (response.exists && response.data) {
							...
							if (data.ADA == 'YES') {
								...
								Swal.fire('Info', 'Stok di TGZ Kosong!', 'info');
								$('#kd_brg').val('');
								$('#info-barang').text('');
							}
						} else {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						Swal.fire({
							...
							window.location.href = "{{ route('tspkoketgz') }}";
						});
					}
				}).catch((error) => {
