						...
						<div class="card-header">
							<div class="row">
								<div class="col-md-6">
									...
									<a href="{{ route('tpengembaliankegudang', ['tipe' => $tipe]) }}" class="btn btn-sm btn-danger">
						...
						<div class="card-body">
							...
							<div class="row">
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="date" class="form-control form-control-sm" name="tgl" id="tgl" value="{{ $header->tgl ?? date('Y-m-d') }}"
											required>
									</div>
								</div>
								<div class="col-md-4">
									<div class="form-group">
										<label>Notes</label>
										<input type="text" class="form-control form-control-sm" name="notes" id="notes" value="{{ $header->notes ?? '' }}"
							...
							<div class="row mt-3">
								<div class="col-md-2">
									<div class="form-group">
										<label>
											<input type="checkbox" id="chkBarcode"> Barcode Timbangan
										</label>
									</div>
								</div>
							</div>
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<div class="input-group input-group-sm">
											<input type="text" class="form-control" id="kd_brg" placeholder="Scan barcode atau ketik kode" autocomplete="off">
								...
								<table class="table-bordered table-striped table-sm table" id="table-detail">
									<thead class="thead-dark">
										<tr>
											...
											<th width="10%" class="text-center">Kemasan</th>
											<th width="7%" class="text-center">Qty</th>
											<th width="8%" class="text-center">Type</th>
											<th width="5%" class="text-center">SPL</th>
										...
										@if (!empty($detail) && count($detail) > 0)
											@foreach ($detail as $key => $row)
												<tr data-no-id="{{ $row->no_id ?? 0 }}">
													...
													<td>
														<input type="text" class="form-control form-control-sm" name="detail[{{ $key }}][barcode]"
															value="{{ $row->barcode ?? '' }}" readonly style="background-color: #e9ecef;">
													</td>
													<td>
													...
													<td>
														<input type="text" class="form-control form-control-sm" name="detail[{{ $key }}][jns]" value="{{ $row->jns ?? '' }}"
															readonly style="background-color: #e9ecef;">
													</td>
													<td class="text-center">
														<input type="text" class="form-control form-control-sm text-center" name="detail[{{ $key }}][spl]"
															value="{{ $row->spl ?? '' }}" readonly style="background-color: #e9ecef;">
													</td>
													<td>
														...
														<input type="hidden" name="detail[{{ $key }}][hb]" value="{{ $row->hb ?? 0 }}">
													</td>
													<td class="text-center">
...
@section('javascripts')
	...
	<script>
		$(document).ready(function() {
			...
			// Enter navigation - bisa langsung ketik dan tekan Enter
			$('#kd_brg').on('keydown', function(e) {
				if (e.keyCode == 13) {
					...
					if (val === '') {
						...
						// Jika ada value, proses barang
						processBarang();
					}
				}
			});

			// Auto-process saat input barcode (opsional - bisa dinonaktifkan jika tidak diinginkan)
			// Uncomment baris berikut jika ingin auto-process setelah scan barcode:
			// $('#kd_brg').on('blur', function() {
			//     let val = $(this).val().trim();
			//     if (val !== '' && !window.currentBarang) {
			//         processBarang();
			//     }
			// });
			...
			function searchBarang(q) {
				...
				$.ajax({
					url: "{{ route('tpengembaliankegudang.browse', ['tipe' => $tipe]) }}",
					data: {
						q: q
					},
			...
			function processBarang() {
				...
				let chkBarcode = $('#chkBarcode').is(':checked');
				...
				$.ajax({
					url: "{{ route('tpengembaliankegudang.detail', ['tipe' => $tipe]) }}",
					data: {
						kd_brg: kdBrg,
						barcode: kdBrg,
					...
					success: function(response) {
						if (response.success && response.exists && response.data) {
							...
							let infoText = data.na_brg + ' | Kemasan: ' + (data.ket_kem || '') + ' | Type: ' + (data.jns || '');
							$('#info-text').text(infoText);
							$('#info-barang').show();

							if (chkBarcode && data.qtyscan) {
								$('#qty').val(data.qtyscan);
								$('#ket').focus();
							} else {
								$('#qty').val('').focus();
							}
			...
			function addBarangToGrid() {
				...
				$('#tbody-detail tr').each(function() {
					if ($(this).find('input[name*="[kd_brg]"]').val() == kdBrg) {
						let currentQty = parseNumber($(this).find('.qty').val());
						$(this).find('.qty').val(currentQty + qty);
				...
				if (!exists) {
                    ...
                    <tr data-no-id="0">
                        ...
                        <td>
                            <input type="text" class="form-control form-control-sm" name="detail[${rowIndex}][barcode]" value="${data.barcode || ''}" readonly style="background-color: #e9ecef;">
                        </td>
                        <td>
                        ...
                        <td>
                            <input type="text" class="form-control form-control-sm" name="detail[${rowIndex}][jns]" value="${data.jns || ''}" readonly style="background-color: #e9ecef;">
                        </td>
                        <td class="text-center">
                            <input type="text" class="form-control form-control-sm text-center" name="detail[${rowIndex}][spl]" value="${data.spl || ''}" readonly style="background-color: #e9ecef;">
                        </td>
                        <td>
                            ...
                            <input type="hidden" name="detail[${rowIndex}][hj]" value="${data.hj || 0}">
                            <input type="hidden" name="detail[${rowIndex}][hb]" value="${data.hb || 0}">
                        </td>
                        <td class="text-center">
                            <button type="button" class="btn btn-xs btn-danger btn-delete-row">
                                <i class="fas fa-trash"></i>
			...
			$('#btn-save').click(function(e) {
				...
				Swal.fire({
					...
					preConfirm: () => {
							...
							if (!$(this).find('td').first().attr('colspan')) {
								...
								if (qty > 0) {
									formData.details.push({
										...
										qty: qty,
										jns: $(this).find('input[name*="[jns]"]').val(),
										spl: $(this).find('input[name*="[spl]"]').val(),
						...
						return $.ajax({
							url: "{{ route('tpengembaliankegudang.store', ['tipe' => $tipe]) }}",
							type: 'POST',
							data: formData,
							dataType: 'json'
						});
					...
					if (result.isConfirmed) {
						Swal.fire({
							...
							window.location.href = "{{ route('tpengembaliankegudang', ['tipe' => $tipe]) }}";
						});
					}
				}).catch((error) => {
