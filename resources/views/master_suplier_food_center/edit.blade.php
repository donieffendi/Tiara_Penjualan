...
@section('content')
								...
								<form action="{{ $tipx == 'new' ? url('/sup/store/') : url('/sup/update/' . $header->NO_ID) }}" method="POST" name ="entri" id="entri">
									...
									<div class="tab-content mt-3">
										...
										<div class="col-md-12 form-group row mt-3">
											<div class="col-md-4">
												<button type="button" id='TOPX' onclick="location.href='{{ url('/sup-food-center/edit/?idx=' . $idx . '&tipx=top') }}'"
													class="btn btn-outline-primary">Top</button>
												<button type="button" id='PREVX'
													onclick="location.href='{{ url('/sup-food-center/edit/?idx=' . $header->NO_ID . '&tipx=prev&kodex=' . $header->KODES) }}'"
													class="btn btn-outline-primary">Prev</button>
												<button type="button" id='NEXTX'
													onclick="location.href='{{ url('/sup-food-center/edit/?idx=' . $header->NO_ID . '&tipx=next&kodex=' . $header->KODES) }}'"
													class="btn btn-outline-primary">Next</button>
												<button type="button" id='BOTTOMX' onclick="location.href='{{ url('/sup-food-center/edit/?idx=' . $idx . '&tipx=bottom') }}'"
	...
	@section('footer-scripts')
		<script>
			...
			function hidup() {
				// Fungsi tidak digunakan untuk mode show
			}
			...
			function simpan() {
				// Disabled - mode show only
				alert('Halaman ini hanya untuk melihat data.');
				return false;
			}
		</script>
	@endsection
