        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/kmkirim/store?flagz='.$flagz.'') : url('/kmkirim/update/'.$header->NO_ID.'&flagz='.$flagz.'' ) }}" method="POST" name ="entri" id="entri" >
                            ...
                            <table id="datatable" class="table table-striped table-border">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="150px" style="text-align: center;">
                                           ...
                                           <label for="NO_KIRIM" class="form-label">No. Kirim</label></th>
                                        <th width="550px" style="text-align: center;">Tujuan Kirim</th>
								...
								@foreach ($detail as $detail)
                                    <tr>
										...
										<td>
                                            <input name="NO_KIRIM[]" id="NO_KIRIM{{$no}}" type="text" class="form-control NO_KIRIM "
											value="{{$detail->NO_KIRIM}}" onblur="browseKirim({{$no}})">
                                        </td>

										<td>
                                            <input name="CBG_TUJU[]" id="CBG_TUJU{{$no}}" type="text" class="form-control CBG_TUJU " value="{{$detail->CBG_TUJU}}" readonly required>
                                        </td>
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button hidden type="button" id='TOPX'  onclick="location.href='{{url('/kmkirim/edit/?idx=' .$idx. '&tipx=top&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Top</button>
								<button hidden type="button" id='PREVX' onclick="location.href='{{url('/kmkirim/edit/?idx='.$header->NO_ID.'&tipx=prev&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Prev</button>
								<button hidden type="button" id='NEXTX' onclick="location.href='{{url('/kmkirim/edit/?idx='.$header->NO_ID.'&tipx=next&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Next</button>
								<button hidden type="button" id='BOTTOMX' onclick="location.href='{{url('/kmkirim/edit/?idx=' .$idx. '&tipx=bottom&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
								<button hidden type="button" id='NEWX' onclick="location.href='{{url('/kmkirim/edit/?idx=0&tipx=new&flagz='.$flagz.'' )}}'" class="btn btn-warning">New</button>
								<button hidden type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>
								<button hidden type="button" id='UNDOX' onclick="location.href='{{url('/kmkirim/edit/?idx=' .$idx. '&tipx=undo&flagz='.$flagz.'' )}}'" class="btn btn-info">Undo</button>
							...
							<div class="col-md-3">
								...
								<!-- <button type="button" id='CLOSEX'  onclick="location.href='{{url('/kmkirim?flagz='.$flagz.'' )}}'" class="btn btn-outline-secondary">Close</button> -->
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title" id="browseKirimModalLabel">Cari Item</h5>
    ...
    baris=<?=$no?>;
		...
		loadDataBKirim = function(){
			...
			$.ajax(
			{
				...
				async : false,
				data: {
						'NO_KIRIM': $("#NO_KIRIM"+rowidKirim).val(),
...

				{
					...
					if ( resp.length > 1 )
					{
							...
							for(i=0; i<resp.length; i++){
								...
								dTableBKirim.row.add([
									'<a href="javascript:void(0);" onclick="chooseKirim(\''+resp[i].NO_KIRIM+'\', \''+resp[i].CBG_KIRIM+'\' )">'+resp[i].NO_KIRIM+'</a>',
                                    resp[i].CBG_KIRIM,
								]);
							}
							dTableBKirim.draw();
					...
					{
						$("#NO_KIRIM"+rowidKirim).val(resp[0].NO_KIRIM);
                        $("#CBG_KIRIM"+rowidKirim).val(resp[0].CBG_KIRIM);
					}
				}
			});
			$("#LOADX").hide();
		...
		browseKirim = function(rid){
			rowidKirim = rid;
			$("#NO_KIRIM"+rowidKirim).val("");
			loadDataBKirim();


			if ( $("#NO_KIRIM"+rowidKirim).val() == '' ) {
					$("#browseKirimModal").modal("show");
			}
		}

		chooseKirim = function(NO_KIRIM, CBG_KIRIM){
			$("#NO_KIRIM"+rowidKirim).val(NO_KIRIM);
            $("#CBG_KIRIM"+rowidKirim).val(CBG_KIRIM);
...
///////////////////////////////////////
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
	            	...
	            	loc = "{{ url('/kmkirim/delete/'.$header->NO_ID) }}" + '?flagz=' + encodeURIComponent(flagz) ;
	...
	function closeTrans() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
	        	loc = "{{ url('/kmkirim/') }}" + '?flagz=' + encodeURIComponent(flagz) ;
				window.location = loc ;
			} else {
				Swal.fire({
					icon: 'info',
	...
	function CariBukti() {
		...
		var flagz = "{{ $flagz }}";
		var cari = $("#CARI").val();
		var loc = "{{ url('/kmkirim/edit/') }}" + '?idx={{ $header->NO_ID}}&tipx=search&flagz=' + encodeURIComponent(flagz) + '&buktix=' +encodeURIComponent(cari);
		window.location = loc;
    ...
    function tambah() {
		...
		html=`<tr>
                ...
                <td>
				    <input name='NO_KIRIM[]' data-rowid=${idrow} onblur='browseKirim(${idrow})' id='NO_KIRIM${idrow}' type='text' class='form-control  NO_KIRIM' >
                </td>
                <td>
				    <input name='CBG_KIRIM[]'   id='CBG_KIRIM${idrow}' type='text' class='form-control  CBG_KIRIM' readonly required>
                </td>
