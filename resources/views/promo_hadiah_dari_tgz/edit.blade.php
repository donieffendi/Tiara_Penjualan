					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<h3 class="card-title">Terima Hadiah dari TGZ - {{ $status == 'simpan' ? 'New' : 'Edit' }}</h3>
							</div>
							<div class="card-body">
								<form action="{{ route('phterimahadiahdaritgz.store') }}" method="POST" name="entri" id="entri">
									@csrf
										...
										<div class="card-body">
											<div class="row">
												...
												<div class="col-md-3">
													<div class="form-group row">
														<div class="col-md-4">
															<label for="agenda" class="form-label">No Resi</label>
														</div>
														<div class="col-md-8">
															<input type="text" class="form-control form-control-sm" id="agenda" name="agenda" value="{{ $header->NO_PO ?? '' }}"
																placeholder="No Resi" {{ $status == 'edit' ? 'readonly' : '' }} required>
														</div>
													</div>
												</div>
														...
														<div class="form-group row">
															<div class="col-md-12">
																<div class="form-check">
																	<input class="form-check-input" type="checkbox" id="posted" name="posted"
																		{{ $header && $header->posted == 1 ? 'checked' : '' }} disabled>
														...
														<div class="row">
															...
															<div class="col-md-2">
																<div class="form-group">
																	...
																	<input type="number" class="form-control form-control-sm" id="harga_entry" name="harga_entry" step="0.01" placeholder="0.00"
																		value="0">
																</div>
															</div>
...
@section('footer-scripts')
	...
	<script>
		...
		$(document).ready(function() {
			...
			// No Resi exit handler (matching Delphi txtagendaExit)
			$('#agenda').on('blur', function() {
				var agenda = $(this).val().trim();
				if (agenda && '{{ $status }}' == 'simpan') {
					handleAgendaExit(agenda);
				}
			});
		});
		...
		// Handle agenda exit (matching Delphi txtagendaExit)
		function handleAgendaExit(agenda) {
			if (!agenda) {
				return;
			}
			...
			Swal.fire({
				title: 'Loading...',
				text: 'Mengambil data dari nomor resi',
			...
			$.ajax({
				url: '{{ route('phterimahadiahdaritgz.load-agenda') }}',
				type: 'GET',
				data: {
					agenda: agenda
				},
				success: function(response) {
					...
					if (response.success) {
						...
						// Add detail items from agenda
						if (response.data && response.data.length > 0) {
							response.data.forEach(function(item) {
								addNewRowFromAgenda(item);
							});
							calculateTotal();
							$('#notes').focus();
						} else {
							Swal.fire({
								icon: 'warning',
								title: 'Warning',
								text: 'Tidak ada detail barang pada resi ini'
							});
						}
					} else {
						Swal.fire({
		...
		function handleKdBrghEnter(kd_brgh) {
			if (kd_brgh) {
				$.ajax({
					url: '{{ route('phterimahadiahdaritgz.detail') }}',
					type: 'GET',
					data: {
						kd_brgh: kd_brgh
					},
		...
		function addToDetail() {
			...
			if (existingRow.length > 0) {
				...
				// Add new row
				addNewRow(TEMP_PRODUCT, qty, harga);
			}
		...
		// Add new row from agenda data
		function addNewRowFromAgenda(item) {
			var newRow = `
				<tr data-kd-brgh="${item.KD_BRGH}" data-no-id="0">
					...
					<input type="hidden" name="details[${detailRowIndex}][harga]" value="${item.harga}">
				</tr>`;
		...
		function addNewRow(product, qty, harga) {
			var newRow = `
		...
		function searchProduct(query) {
			$.ajax({
				url: '{{ route('phterimahadiahdaritgz.browse') }}',
				type: 'GET',
				data: {
					q: query
				},
		...
		function simpan() {
			...
			// Show confirmation (matching Delphi MessageBox)
			Swal.fire({
				title: 'Konfirmasi',
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						url: '{{ route('phterimahadiahdaritgz.store') }}',
						type: 'POST',
						...
						success: function(response) {
							...
							if (response.success) {
								Swal.fire({
									...
									window.location.href = '{{ route('phterimahadiahdaritgz') }}';
								});
							} else {
								Swal.fire({
									title: 'Error!',
