...
<div class="content-wrapper">
        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/hterima/store?flagz='.$flagz.'') : url('/hterima/update/'.$header->NO_ID.'&flagz='.$flagz.'' ) }}" method="POST" name ="entri" id="entri" >
                        ...
                        <div class="tab-content mt-3">
							...
							<div class="form-group row">
								...
								<div class="col-md-2 input-group" >
									<input type="text" class="form-control NO_KIRIM" id="NO_KIRIM" name="NO_KIRIM" placeholder="Pilih No. Kirim"value="{{$header->NO_KIRIM}}" style="text-align: left" readonly >
									<button type="button" class="btn btn-primary" onclick="browseKirim()"><i class="fa fa-search"></i></button>
								</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-2">
									<select name="CBG_DARI" id="CBG_DARI" class="form-control CBG_DARI" style="width: 300px">
										<option value="">--Pilih Pengirim--</option>
										@foreach($pilihcbg as $pilihcbgD)
											<option value="{{$pilihcbgD->EXT}}" {{ $header->CBG_DARI == $pilihcbgD->EXT ? 'selected' : '' }}>{{ $pilihcbgD->EXT }}</option>
										@endforeach
									</select>
								</div>   
                            ...
                            <table id="datatable" class="table table-striped table-border">
								...
								@foreach ($detail as $detail)		
                                    <tr>
										...
										<td>
											<input name="QTY1[]" onclick="select()" onkeyup="hitung()" value="{{$detail->QTY1}}" id="QTY1{{$no}}" type="text" style="text-align: right"  class="form-control QTY1 text-primary" readonly required>
										</td>

										<td>
											<input name="QTY[]" onclick="select()" onkeyup="hitung()" value="{{$detail->QTY}}" id="QTY{{$no}}" type="text" style="text-align: right"  class="form-control QTY text-primary" required>
										</td>
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button hidden type="button" id='TOPX'  onclick="location.href='{{url('/hterima/edit/?idx=' .$idx. '&tipx=top&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Top</button>
								<button hidden type="button" id='PREVX' onclick="location.href='{{url('/hterima/edit/?idx='.$header->NO_ID.'&tipx=prev&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Prev</button>
								<button hidden type="button" id='NEXTX' onclick="location.href='{{url('/hterima/edit/?idx='.$header->NO_ID.'&tipx=next&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Next</button>
								<button hidden type="button" id='BOTTOMX' onclick="location.href='{{url('/hterima/edit/?idx=' .$idx. '&tipx=bottom&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
								<button hidden type="button" id='NEWX' onclick="location.href='{{url('/hterima/edit/?idx=0&tipx=new&flagz='.$flagz.'' )}}'" class="btn btn-warning">New</button>
								<button hidden type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>                    
								<button hidden type="button" id='UNDOX' onclick="location.href='{{url('/hterima/edit/?idx=' .$idx. '&tipx=undo&flagz='.$flagz.'' )}}'" class="btn btn-info">Undo</button>  
							...
							<div class="col-md-3">
								...
								<!-- <button type="button" id='CLOSEX'  onclick="location.href='{{url('/hterima?flagz='.$flagz.'' )}}'" class="btn btn-outline-secondary">Close</button> -->
	...
	<div class="modal fade" id="browseKirimModal" tabindex="-1" role="dialog" aria-labelledby="browseKirimModalLabel" aria-hidden="true">
	  <div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title" id="browseKirimModalLabel">Cari Bukti Kirim</h5>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
			</button>
		  </div>
...
<script>
    ...
    baris=<?=$no?>;
		...
		//		CHOOSE Kirim
		var dTableBKirim;
		loadDataBKirim = function(){
		
			$.ajax(
			{
				type: 'GET', 		
				url: '{{url('hkirim/browse')}}',
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBKirim.row.add([
							'<a href="javascript:void(0);" onclick="chooseKirim(\''+resp[i].NO_KIRIM+'\', \''+resp[i].CBG+'\' )">'+resp[i].NO_KIRIM+'</a>',
							resp[i].CBG,
						]);
					}
					dTableBKirim.draw();
		...
		dTableBKirim = $("#table-bkirim").DataTable({
			
		});
		
		browseKirim = function(){
			loadDataBKirim();
			$("#browseKirimModal").modal("show");
		}
		
		chooseKirim = function( NO_KIRIM, CBG ){

			$("#NO_KIRIM").val(NO_KIRIM);
			$("#CBG_DARI").val(CBG);		
			$("#browseKirimModal").modal("hide");
			
			getKirimd(NO_KIRIM);
		}
		
		$("#NO_KIRIM").keypress(function(e){
				...
				success: function( resp )
				{
					...
					for(i=0; i<resp.length; i++){
						html+=`<tr>
									...
									<td>
										<input name='QTY1[]' onclick='select()' onblur='hitung()' id='QTY1${i}' value="${resp[i].QTY1}" type='text' style='text-align: right' class='form-control QTY1 text-primary' readonly>
									</td>
									<td>
					...
					$('#detailKirimd').html(html);

					$(".QTY").autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
					$(".QTY1").autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
...
//////////////////////////////////////////////////////////////////
	...
	function cekDetail2(){
		...
		$(".QTY").each(function() {
			...
			let z = $(this).closest('tr');
			var QTY1X = parseFloat(z.find('.QTY1').val().replace(/,/g, ''));
        ...
        var check = '0';
			...
			if (cekDetail2()) {
				...
				Swal.fire({
					icon: 'warning',
					title: 'Warning',
					text: 'Data Kirim dan Terima Tidak Sama.'
				});
	...
	function hidup() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#QTY1" + i.toString()).attr("readonly", true);
	...
	function mati() {
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#QTY1" + i.toString()).attr("readonly", false);
	...
	// 	{
	// 		window.location ="{{url('/hterima/delete/'.$header->NO_ID .'/?flagz='.$flagz.'' )}}";
	// 		//return true;
	// 	} 
	// 	return false;
	// }
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
	            	...
	            	loc = "{{ url('/hterima/delete/'.$header->NO_ID) }}" + '?flagz=' + encodeURIComponent(flagz) ;
	...
	function closeTrans() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
	        	loc = "{{ url('/hterima/') }}" + '?flagz=' + encodeURIComponent(flagz) ;
				window.location = loc ;
			} else {
				Swal.fire({
					icon: 'info',
	...
	function CariBukti() {
		...
		var flagz = "{{ $flagz }}";
		var cari = $("#CARI").val();
		var loc = "{{ url('/hterima/edit/') }}" + '?idx={{ $header->NO_ID}}&tipx=search&flagz=' + encodeURIComponent(flagz) + '&buktix=' +encodeURIComponent(cari);
		window.location = loc;
    ...
    function tambah() {
		...
		html=`<tr>
				...
				<td>
		            <input name='QTY1[]' onclick='select()' onblur='hitung()' value='0' id='QTY1${idrow}' type='text' style='text-align: right' class='form-control QTY1 text-primary' readonly required >
                </td>
