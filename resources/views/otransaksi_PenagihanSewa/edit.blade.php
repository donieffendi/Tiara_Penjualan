					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								...
								<form action="{{ $tipx == 'new' ? url('/tpenagihansewa/store/') : url('/tpenagihansewa/update/' . $header->NO_ID) }}" method="POST"
									name="entri" id="entri">
									@csrf
									@if ($tipx != 'new')
										@method('POST')
									...
									<div class="tab-content mt-3">
										...
										<div class="row">
											...
											<div class="col-md-6 pl-4">
												<div class="form-group row align-items-center">
													...
													<div class="col-md-8">
														<input type="text" class="form-control" id="GOL" name="GOL" value="{{ $header->GOL ?? '' }}" readonly>
													</div>
												</div>
												<div class="form-group row align-items-center">
													<div class="col-md-4">
														<label for="TYP" class="form-label">JENIS PAJAK</label>
													</div>
													<div class="col-md-8">
														<input type="text" class="form-control" id="TYP" name="TYP" value="{{ $header->TYP ?? '' }}" readonly>
													</div>
												</div>
												<div class="form-group row align-items-center">
													<div class="col-md-4">
													...
													<div class="col-md-8">
														<input type="text" class="form-control" id="PRODUK" name="PRODUK" value="{{ $header->PRODUK ?? '' }}"
												...
												<div class="form-group row align-items-center">
													...
													<div class="col-md-8">
														<textarea class="form-control" id="NOTES" name="NOTES" rows="2" placeholder="Catatan">{{ $header->NOTES ?? '' }}</textarea>
													</div>
												</div>
											</div>
										</div>
													...
													<tbody id="detail-body">
														@if (!empty($detail))
															@foreach ($detail as $index => $item)
																<tr data-index="{{ $index }}">
																	<td>{{ $index + 1 }}</td>
																	<td><input type="text" class="form-control" name="detail[{{ $index }}][URAIAN]" value="{{ $item->URAIAN }}"
																			placeholder="Uraian"></td>
																	<td><input type="number" class="form-control text-right" name="detail[{{ $index }}][TOTAL]" value="{{ $item->TOTAL }}"
																			step="0.01" onchange="calculateTotal()"></td>
														...
														@else
															<tr data-index="0">
																<td>1</td>
																<td><input type="text" class="form-control" name="detail[0][URAIAN]" placeholder="Uraian"></td>
																<td><input type="number" class="form-control text-right" name="detail[0][TOTAL]" step="0.01" onchange="calculateTotal()"></td>
																<td><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)">Del</button></td>
															</tr>
														@endif
													</tbody>
											...
											<div class="col-md-4">
												<div class="summary-panel">
													<div class="summary-item">
														<label>DPP:</label>
														<input type="number" class="form-control d-inline-block" id="DPP" name="DPP" value="{{ $header->DPP ?? 0 }}"
															step="0.01" readonly>
													</div>
													<div class="summary-item">
														<label>PPN:</label>
														<input type="number" class="form-control d-inline-block" id="PPN" name="PPN" value="{{ $header->PPN ?? 0 }}"
															step="0.01" onchange="calculateTotal()">
													</div>
													<div class="summary-item">
														<label>PPH 1:</label>
														<input type="number" class="form-control d-inline-block" id="PPH1" name="PPH1" value="{{ $header->PPH1 ?? 0 }}"
															step="0.01" onchange="calculateTotal()">
													</div>
													<div class="summary-item">
														<label>PPH 2:</label>
														<input type="number" class="form-control d-inline-block" id="PPH2" name="PPH2" value="{{ $header->PPH2 ?? 0 }}"
															step="0.01" onchange="calculateTotal()">
													</div>
													<div class="summary-item">
														<label style="color: blue;">BRUTO:</label>
														<input type="number" class="form-control d-inline-block" id="BRUTO" name="BRUTO"
															value="{{ ($header->DPP ?? 0) + ($header->PPN ?? 0) }}" step="0.01" readonly style="font-weight: bold; color: blue;">
													</div>
													<div class="summary-item">
										...
										<div class="col-md-12 form-group row mt-3">
											<div class="col-md-4">
												<button type="button" id='TOPX' onclick="location.href='{{ url('/tpenagihansewa/edit/?idx=' . $idx . '&tipx=top') }}'"
													class="btn btn-outline-primary">Top</button>
												<button type="button" id='PREVX'
													onclick="location.href='{{ url('/tpenagihansewa/edit/?idx=' . ($header->NO_ID ?? 0) . '&tipx=prev&kodex=' . ($header->NO_BUKTI ?? '')) }}'"
													class="btn btn-outline-primary">Prev</button>
												<button type="button" id='NEXTX'
													onclick="location.href='{{ url('/tpenagihansewa/edit/?idx=' . ($header->NO_ID ?? 0) . '&tipx=next&kodex=' . ($header->NO_BUKTI ?? '')) }}'"
													class="btn btn-outline-primary">Next</button>
												<button type="button" id='BOTTOMX' onclick="location.href='{{ url('/tpenagihansewa/edit/?idx=' . $idx . '&tipx=bottom') }}'"
													class="btn btn-outline-primary">Bottom</button>
											</div>
											<div class="col-md-5">
												<button type="button" id='NEWX' onclick="location.href='{{ url('/tpenagihansewa/edit/?idx=0&tipx=new') }}'"
													class="btn btn-warning">New</button>
												<button type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>
												<button type="button" id='UNDOX' onclick="location.href='{{ url('/tpenagihansewa/edit/?idx=' . $idx . '&tipx=undo') }}'"
...
@section('footer-scripts')
	...
	<script>
		...
		function addRow() {
			...
			var newRow = `
				<tr data-index="${detailRowIndex}">
					...
					<td><input type="text" class="form-control" name="detail[${detailRowIndex}][URAIAN]" placeholder="Uraian"></td>
					<td><input type="number" class="form-control text-right" name="detail[${detailRowIndex}][TOTAL]" step="0.01" onchange="calculateTotal()"></td>
					<td><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)">Del</button></td>
				</tr>
			`;
		...
		function calculateTotal() {
			...
			var pph1 = parseFloat($('#PPH1').val()) || 0;
			var pph2 = parseFloat($('#PPH2').val()) || 0;

			// Calculate DPP from detail rows
			$('#detail-body input[name*="[TOTAL]"]').each(function() {
				dpp += parseFloat($(this).val()) || 0;
			});

			// Update DPP
			$('#DPP').val(dpp.toFixed(2));

			// Calculate automatic PPN if enabled
			if ($('#TYP').val() === 'PKP' || $('#TYP').val().includes('PKP')) {
				ppn = dpp * 0.11; // 11% PPN
				$('#PPN').val(ppn.toFixed(2));
			}

			// Calculate BRUTO (DPP + PPN)
			var bruto = dpp + ppn;
			$('#BRUTO').val(bruto.toFixed(2));

			// Calculate TOTAL (BRUTO - PPH1 - PPH2)
			var total = bruto - pph1 - pph2;
		...
		function getCustomerData(kodec) {
			$.ajax({
				type: "GET",
				url: "{{ url('tpenagihansewa/get-select-kodes') }}",
				data: {
					kodec: kodec
				},
				success: function(data) {
					if (data.length > 0) {
						...
						$('#GOL').val(customer.golongan);
						$('#TYP').val(customer.jenispjk);
						calculateTotal(); // Recalculate in case tax type changed
					} else {
						Swal.fire({
							icon: 'warning',
		...
		function searchCustomers(query) {
			$.ajax({
				type: "GET",
				url: "{{ url('tpenagihansewa/browsesupz') }}",
				data: {
					q: query
				},
				success: function(data) {
		...
		function hidup() {
			...
			$("#PRODUK").attr("readonly", false);
			$("#TBAYAR").attr("disabled", false);
			$("#NOTES").attr("readonly", false);
			$("#PPN").attr("readonly", false);
			$("#PPH1").attr("readonly", false);
			$("#PPH2").attr("readonly", false);
		...
		function mati() {
			...
			$("#PPH1").attr("readonly", true);
			$("#PPH2").attr("readonly", true);
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					Swal.fire({
						...
						var loc = "{{ url('/tpenagihansewa/delete/' . ($header->NO_ID ?? 0)) }}";
						window.location = loc;
					});
				}
			});
		...
		function CariBukti() {
			var cari = $("#CARI").val();
			var loc = "{{ url('/tpenagihansewa/edit/') }}" + '?idx={{ $header->NO_ID ?? 0 }}&tipx=search&kodex=' + encodeURIComponent(cari);
			window.location = loc;
		}
		...
		function simpan() {
			...
			$('#detail-body input[name*="[URAIAN]"]').each(function() {
				if ($(this).val().trim() !== '') {
					hasValidDetail = true;
			...
			if (!hasValidDetail) {
				Swal.fire({
					icon: 'warning',
					title: 'Warning',
					text: 'Minimal satu detail uraian harus diisi.'
				});
				return;
			}
