					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								...
								<form action="{{ $tipx == 'new' ? url('/TPanitiaAcara/store/') : url('/TPanitiaAcara/update/' . ($header->no_bukti ?? '')) }}" method="POST"
									name="entri" id="entri">
									@csrf
									@if ($tipx != 'new')
										@method('POST')
									...
									<div class="tab-content mt-3">
										<!-- Main Form Content - Three Column Layout -->
										<div class="row">
											<!-- Left Column -->
											<div class="col-md-4">
											...
											<div class="col-md-4">
												<div class="form-group row align-items-center">
													<div class="col-md-4">
														<label for="PANITIA" class="form-label">PANITIA</label>
													</div>
													<div class="col-md-8">
														<input type="text" class="form-control" id="PANITIA" name="PANITIA" value="{{ $header->panitia ?? '' }}" placeholder="Nama Panitia">
													</div>
												</div>
												<div class="form-group row align-items-center">
													<div class="col-md-4">
													...
													<div class="col-md-8">
														<input type="text" class="form-control" id="SEKSI" name="SEKSI" value="{{ $header->seksi ?? '' }}" placeholder="Nama Seksi">
													</div>
												</div>
												<div class="form-group row align-items-center">
													<div class="col-md-4">
														<label for="KEPL" class="form-label">KEPALA</label>
													</div>
													<div class="col-md-8">
														<input type="text" class="form-control" id="KEPL" name="KEPL" value="{{ $header->kepl ?? '' }}"
												...
												<table class="table-striped detail-table table" id="detail-table">
													<thead class="table-dark">
														<tr>
															...
															<th width="90px">Tanggal Publish</th>
													...
													<tbody id="detail-body">
															...
															@foreach ($detail as $index => $item)
																<tr data-index="{{ $index }}">
																	...
																	<td><input type="text" class="form-control" name="detail[{{ $index }}][NA_BRG]" value="{{ $item->na_brg }}"
																		...
																		@if (isset($item->no_id))
																			<input type="hidden" name="detail[{{ $index }}][NO_ID]" value="{{ $item->no_id }}">
																		@endif
																	</td>
																	...
																	<td><input type="date" class="form-control" name="detail[{{ $index }}][TG_PBL]" value="{{ $item->tg_pbl }}"></td>
																	<td><input type="text" class="form-control" name="detail[{{ $index }}][AKUNT]" value="{{ $item->akunt }}"
																			placeholder="Akun"></td>
																	<td>
																		<textarea class="form-control" name="detail[{{ $index }}][KET_DETAIL]" placeholder="Keterangan">{{ $item->ket_detail ?? '' }}</textarea>
																	</td>
														...
														@else
															<tr data-index="0">
																...
																<td><input type="text" class="form-control" name="detail[0][SATUAN]" placeholder="Satuan"></td>
																<td><input type="text" class="form-control" name="detail[0][UKURAN]" placeholder="Ukuran"></td>
																...
																<td><input type="date" class="form-control" name="detail[0][TG_PBL]"></td>
																<td><input type="text" class="form-control" name="detail[0][AKUNT]" placeholder="Akun"></td>
																<td>
																	<textarea class="form-control" name="detail[0][KET_DETAIL]" placeholder="Keterangan"></textarea>
																</td>
																<td><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)">Del</button></td>
															</tr>
														@endif
													...
													<table class="table" style="margin-bottom: 0;">
														<tbody>
															<tr>
																...
																<td width="80px"></td> <!-- Satuan -->
																<td width="80px"></td> <!-- Ukuran -->
																...
																<td width="90px"></td> <!-- Tanggal Publish -->
																<td width="100px"></td> <!-- Akun -->
																<td width="120px"></td> <!-- Keterangan -->
										...
										<div class="col-md-12 form-group row mt-3">
											<div class="col-md-4">
												<button type="button" id='TOPX' onclick="location.href='{{ url('/TPanitiaAcara/edit/?idx=' . $idx . '&tipx=top') }}'"
													class="btn btn-outline-primary">Top</button>
												<button type="button" id='PREVX'
													onclick="location.href='{{ url('/TPanitiaAcara/edit/?idx=' . ($header->no_bukti ?? '') . '&tipx=prev&kodex=' . ($header->no_bukti ?? '')) }}'"
													class="btn btn-outline-primary">Prev</button>
												<button type="button" id='NEXTX'
													onclick="location.href='{{ url('/TPanitiaAcara/edit/?idx=' . ($header->no_bukti ?? '') . '&tipx=next&kodex=' . ($header->no_bukti ?? '')) }}'"
													class="btn btn-outline-primary">Next</button>
												<button type="button" id='BOTTOMX' onclick="location.href='{{ url('/TPanitiaAcara/edit/?idx=' . $idx . '&tipx=bottom') }}'"
													class="btn btn-outline-primary">Bottom</button>
											</div>
											<div class="col-md-5">
												<button type="button" id='NEWX' onclick="location.href='{{ url('/TPanitiaAcara/edit/?idx=0&tipx=new') }}'"
													class="btn btn-warning">New</button>
												<button type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>
												<button type="button" id='UNDOX' onclick="location.href='{{ url('/TPanitiaAcara/edit/?idx=' . $idx . '&tipx=undo') }}'"
	...
	<script>
		...
		function addRow() {
			...
			var newRow = `
				<tr data-index="${detailRowIndex}">
					...
					<td>
						<input type="date" class="form-control" name="detail[${detailRowIndex}][TG_PBL]">
					</td>
					<td>
						<input type="text" class="form-control" name="detail[${detailRowIndex}][AKUNT]" placeholder="Akun">
					</td>
					<td>
						<textarea class="form-control" name="detail[${detailRowIndex}][KET_DETAIL]" placeholder="Keterangan"></textarea>
					</td>
					<td><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)">Del</button></td>
				</tr>
			`;
		...
		function updateRowNumbers() {
			$('#detail-body tr').each(function(index) {
				...
				$(this).find('input[name*="[TG_PBL]"]').attr('name', `detail[${index}][TG_PBL]`);
				$(this).find('input[name*="[AKUNT]"]').attr('name', `detail[${index}][AKUNT]`);
				$(this).find('textarea[name*="[KET_DETAIL]"]').attr('name', `detail[${index}][KET_DETAIL]`);
				$(this).find('input[name*="[NO_ID]"]').attr('name', `detail[${index}][NO_ID]`);
		...
		function getDepartmentData(kdDept) {
			$.ajax({
				type: "GET",
				url: "{{ url('TPanitiaAcara/get-select-kodes') }}",
				data: {
					kodes: kdDept
				},
		...
		function hidup() {
			...
			$("#PANITIA").attr("readonly", false);
			$("#SEKSI").attr("readonly", false);
			$("#KEPL").attr("readonly", false);
		...
		function mati() {
			...
			$("#PANITIA").attr("readonly", true);
			$("#SEKSI").attr("readonly", true);
			$("#KEPL").attr("readonly", true);
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					Swal.fire({
						...
						var loc = "{{ url('/TPanitiaAcara/delete/' . ($header->no_bukti ?? '')) }}";
						window.location = loc;
					});
				}
			});
		...
		function CariBukti() {
			var cari = $("#CARI").val();
			var loc = "{{ url('/TPanitiaAcara/edit/') }}" + '?idx={{ $header->no_bukti ?? '' }}&tipx=search&kodex=' + encodeURIComponent(cari);
			window.location = loc;
		}
		...
		function simpan() {
			...
			var formData = new FormData(document.getElementById('entri'));

			$.ajax({
				type: 'POST',
				url: document.getElementById('entri').action,
				data: formData,
				processData: false,
				contentType: false,
				success: function(response) {
					...
					if (response.success) {
						Swal.fire({
							...
							if ($tipx == 'new') {
								var loc = "{{ url('/TPanitiaAcara/edit/') }}" + '?idx=' + response.no_bukti + '&tipx=edit';
								window.location = loc;
							} else {
								location.reload();
							}
