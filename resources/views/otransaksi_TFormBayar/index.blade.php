										...
										<div class="modal-content">
											...
											<div class="modal-body">
												...
												<form id="columnToggleForm">
													...
													<div class="form-check">
														<input class="form-check-input column-checkbox" type="checkbox" value="11" id="columnKLB" checked>
														<label class="form-check-label" for="columnKLB">KLB</label>
													</div>
													<div class="form-check">
									...
									<div class="col-md-12">
										<div class="card">
											<div class="card-header">
												<h5>Print Instruksi Pembayaran</h5>
											</div>
											<div class="card-body">
												<form id="printForm" method="POST">
													...
													<div class="row">
														<div class="col-md-3">
															...
															<input type="text" class="form-control" name="txtbukti1" id="txtbukti1" placeholder="TG2501-0001">
														</div>
														<div class="col-md-3">
															<label for="txtbukti2">No Bukti Sampai:</label>
															<input type="text" class="form-control" name="txtbukti2" id="txtbukti2" placeholder="TG2501-0010">
														</div>
														<div class="col-md-3">
															<label for="cbKertas">Jenis Kertas:</label>
															<select class="form-control" name="cbKertas" id="cbKertas">
																<option value="PENDEK">PENDEK (139.5mm)</option>
																<option value="PANJANG">PANJANG (279.0mm)</option>
															</select>
														</div>
														<div class="col-md-3 d-flex align-items-end">
															<button type="button" class="btn btn-info mr-2" onclick="printRange()">
												...
												<form id="postingForm" method="POST">
													...
													<div class="row">
														...
														<div class="col-md-4 d-flex align-items-end">
															<button type="button" class="btn btn-warning mr-2" onclick="postingData()">
																<i class="fas fa-check"></i> Posting
															</button>
															<button type="button" class="btn btn-danger" onclick="unpostingData()">
																<i class="fas fa-times"></i> Unposting
															</button>
														</div>
														<div class="col-md-4">
															<small class="text-muted">Pilih data pada tabel dengan checkbox untuk posting/unposting</small>
														</div>
													</div>
												</form>
											</div>
								...
								<table class="table-striped table-border table-hover nowrap datatable table table-fixed" id="datatable">
									<thead class="table-dark">
										<tr>
											...
											<th scope="col" style="text-align: center">KLB</th>
...
@section('javascripts')
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("div.test_btn").html(
				'<a class="btn btn-lg btn-md btn-success" href="{{ url('tformbayar/edit?flagz=' . $flagz . '&idx=0&tipx=new') }}"> <i class="fas fa-plus fa-sm md-3" ></i></a'
			);
			...
			window.toggleButton = function(button) {
				...
				if (button.innerText === '+') {
					...
					$.ajax({
						url: '{{ route('get-detail-tformbayar') }}', // Define the route to fetch detail data
						method: 'GET',
						data: {
							no_bukti: no_bukti
                        ...
                        `;
							...
							response.forEach((item, index) => {
                                ...
                                <tr>
                                    ...
                                    <td><div style="background-color: #f7d8b4; padding: 0.5rem; text-align: center">${item.no_trm || ''}</div></td>
                                    <td><div style="background-color: #f7d8b4; padding: 0.5rem; text-align: center">${item.tgl_trm || ''}</div></td>
                                    ...
                                    <td><div style="background-color: #f7d8b4; padding: 0.5rem; text-align: center">${item.notes || ''}</div></td>
                                </tr>
                            `;
							});
		...
		function printRange() {
			...
			const cbKertas = document.getElementById('cbKertas').value;
			...
			// Validation: first character must match
			if (txtbukti1.charAt(0) !== txtbukti2.charAt(0)) {
				Swal.fire({
					title: 'Error!',
					text: 'Karakter pertama No Bukti harus sama!',
					icon: 'error',
					confirmButtonText: 'OK'
				});
				return;
			...
			Swal.fire({
				title: 'Memproses...',
				text: 'Sedang memproses print range',
			...
			// Make AJAX request
			$.ajax({
				url: '{{ route('tformbayar_print_range') }}',
				method: 'POST',
				data: {
					_token: '{{ csrf_token() }}',
					txtbukti1: txtbukti1,
		...
		function printReport() {
			...
			window.open('{{ route('tformbayar_jasper') }}', '_blank');
			Swal.close();
		}
		...
		function printSingle(no_bukti) {
			...
			$.ajax({
				url: '{{ route('tformbayar_print_single') }}',
				method: 'POST',
				data: {
					_token: '{{ csrf_token() }}',
					no_bukti: no_bukti
	...
	<script>
		...
		// Unposting Function
		function unpostingData() {
			const checkedBoxes = $('.cek:checked');
			if (checkedBoxes.length === 0) {
				Swal.fire({
					title: 'Warning!',
					text: 'Pilih data yang akan di-unposting!',
					icon: 'warning',
					confirmButtonText: 'OK'
				});
				return;
			...
			Swal.fire({
				...
				text: `Apakah Anda yakin akan unposting ${checkedBoxes.length} data?`,
				icon: 'question',
				showCancelButton: true,
				confirmButtonColor: '#3085d6',
				cancelButtonColor: '#d33',
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						url: '{{ route('tformbayar_unposting') }}',
						method: 'POST',
						data: {
							_token: '{{ csrf_token() }}',
							cek: checkedIds
						...
						success: function(response) {
							Swal.fire({
								title: 'Success!',
								text: 'Data berhasil di-unposting',
								icon: 'success',
								confirmButtonText: 'OK'
							}).then(() => {
								window.location.reload();
						...
						error: function(xhr) {
							let errorMessage = 'Terjadi kesalahan saat unposting';
							if (xhr.responseJSON && xhr.responseJSON.message) {
								errorMessage = xhr.responseJSON.message;
							}
