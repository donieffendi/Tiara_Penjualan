                ...
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/ambil/store?flagz='.$flagz.'') : url('/ambil/update/'.$header->NO_ID.'&flagz='.$flagz.'' ) }}" method="POST" name ="entri" id="entri" >
                        ...
                        <div class="tab-content mt-3">
                            ...
                            <div class="form-group row">
								...
								<div class="col-md-1" align="right">
									...
									<select name="CBG" id="CBG" class="form-control CBG">
							...
							<div class="form-group row">
								<div class="col-md-1" align="right">
                                    ...
                                    <label for="NO_JUAL" class="form-label">No. Kitir#</label>
                                </div>
                               	<div class="col-md-2 input-group" >
                                  <input type="text" class="form-control NO_JUAL" id="NO_JUAL" name="NO_JUAL" placeholder="Masukkan Kitir"value="{{$header->NO_JUAL}}" style="text-align: left" readonly >
        						  <button type="button" class="btn btn-primary" onclick="getJuald()"><i class="fa fa-search"></i></button>
                                </div>
                            </div>
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button hidden type="button" id='TOPX'  onclick="location.href='{{url('/ambil/edit/?idx=' .$idx. '&tipx=top&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Top</button>
								<button hidden type="button" id='PREVX' onclick="location.href='{{url('/ambil/edit/?idx='.$header->NO_ID.'&tipx=prev&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Prev</button>
								<button hidden type="button" id='NEXTX' onclick="location.href='{{url('/ambil/edit/?idx='.$header->NO_ID.'&tipx=next&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Next</button>
								<button hidden type="button" id='BOTTOMX' onclick="location.href='{{url('/ambil/edit/?idx=' .$idx. '&tipx=bottom&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
								<button hidden type="button" id='NEWX' onclick="location.href='{{url('/ambil/edit/?idx=0&tipx=new&flagz='.$flagz.'' )}}'" class="btn btn-warning">New</button>
								<button hidden type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>                    
								<button hidden type="button" id='UNDOX' onclick="location.href='{{url('/ambil/edit/?idx=' .$idx. '&tipx=undo&flagz='.$flagz.'' )}}'" class="btn btn-info">Undo</button>  
...
//////////////////////////////////////////////
			...
			$.ajax(
			{
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBCust.row.add([
							'<a href="javascript:void(0);" onclick="chooseCustomer(\''+resp[i].KODEC+'\',  \''+resp[i].NAMAC+'\',  \''+resp[i].HARI+'\', \''+resp[i].ALAMAT+'\',  \''+resp[i].KOTA+'\',  \''+resp[i].PKP+'\')">'+resp[i].KODEC+'</a>',
							resp[i].NAMAC,
							resp[i].ALAMAT,
							resp[i].KOTA,
							resp[i].PKP2,
		...
		chooseCustomer = function(KODEC,NAMAC, HARI, ALAMAT, KOTA, PKP){
			$("#KODEC").val(KODEC);
			$("#NAMAC").val(NAMAC);
			$("#HARI").val(HARI);
			$("#ALAMAT").val(ALAMAT);
			...
			$.ajax(
			{
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBJual.row.add([
							'<a href="javascript:void(0);" onclick="chooseJual(\''+resp[i].NO_BUKTI+'\'    )">'+resp[i].NO_BUKTI+'</a>',
							resp[i].TGL,
						]);
					}
					dTableBJual.draw();
...
/////////////////////////////////////////////////////////////////
	...
	function getJuald(bukti)
	{
		...
		var bukti = $('#NO_JUAL').val();

		$.ajax(
			{
				type: 'POST',    
				url: "http://192.168.0.2/admin-apf-app/api/tukar_hadiah",
				data: JSON.stringify({
					status: "new",
					outlet: $('#CBG').val(),
					kitir: bukti,
				...
				success: function( resp )
				{
					...
					for(i=0; i<data.length; i++){
						html+=`<tr>
                                    <td><input name='REC[]' id='REC${i}' value=${data[i].REC+1} type='text' class='REC form-control' onkeypress='return tabE(this,event)' readonly></td>
                                    <td><input name='KD_BRG[]' data-rowid=${i} id='KD_BRG${i}' value="${data[i].KD_BRGH}" type='text' class='form-control KD_BRG' readonly></td>
                                    <td><input name='NA_BRG[]' data-rowid=${i} id='NA_BRG${i}' value="${data[i].NA_BRGH}" type='text' class='form-control  NA_BRG' readonly></td>
                                    
									<td>
										<input name='QTY[]' onclick='select()' onkeyup='hitung()' id='QTY${i}' value="${data[i].QTY}" type='text' style='text-align: right' class='form-control QTY text-primary' readonly >
									</td>
									<td>
