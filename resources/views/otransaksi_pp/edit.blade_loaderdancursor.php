...
<style>
	...
	input.form-control:read-only {
		background-color: rgba(0, 157, 255, 0.3) !important; /* atau warna lain */
	...
	@keyframes shadowPulse {
		33% {
			background: #FFF;
			box-shadow: -48px 0 #009dff, 48px 0 #FFF;
		}
		66% {
			background: #009dff;
			box-shadow: -48px 0 #FFF, 48px 0 #FFF;
		...
		100% {
			background: #FFF;
			box-shadow: -48px 0 #FFF, 48px 0 #009dff;
		}
	}
                ...
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/pp/store?flagz='.$flagz.'&golz='.$golz.'') : url('/pp/update/'.$header->NO_ID.'&flagz='.$flagz.'&golz='.$golz.'' ) }}" method="POST" name ="entri" id="entri" >
                        ...
                        <div class="tab-content mt-3">
							...
							<div class="loader-overlay" id="OVERLAY">
					...
					<!--		<div style="overflow-y:scroll; " class="col-md-12 scrollable" align="right"> -->
                                ...
                                <thead>
                                    <tr>
										...
										<th width="200px" style="text-align:center">Barang
                                    ...
                                    <tr>
										...
										<td>
											<div class="input-group">
												<input name="KD_BRG[]" id="KD_BRG{{$no}}" type="text" class="form-control KD_BRG" value="{{$detail->KD_BRG}}" onblur="browseBarang({{$no}})">
												<div class="input-group-append">
													<button type="button" class="btn btn-outline-primary" onclick="browseBarang({{$no}})">
...
//		CHOOSE Supplier
		...
		browseBarang = function(rid){
			...
			if ($("#NA_BRG"+rowidBarang).val() == '') {
				...
				// Tunggu modal benar-benar ditampilkan dulu, baru fokus ke search
				$('#browseBarangModal').on('shown.bs.modal', function () {
					setTimeout(function() {
						$('.dataTables_filter input').focus();
					}, 200); // jeda kecil untuk memastikan modal sudah sepenuhnya muncul
				});
			}
		}
...
///////////////////////////////////////
	...
	function cekDetail2(){
 	...
 	function simpan() {
		...
		const btn = document.getElementById("SAVEX");
		...
		html=`<tr>
				...
				<td>
                    <div class="input-group">
                        ...
                        <div class="input-group-append">
                            <button type="button" class="btn btn-outline-primary" onclick="browseBarang(${idrow})">
                                <i class="fas fa-search"></i> <!-- Ikon pencarian -->
                            </button>
                        </div>
                    </div>
