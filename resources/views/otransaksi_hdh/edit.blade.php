					...
					<div class="col-12">
						<div class="card">
							<div class="card-body">
								...
								<form action="{{ $tipx == 'new' ? url('/hdh/store?flagz=' . $flagz . '') : url('/hdh/update/' . $header->NO_ID . '&flagz=' . $flagz . '') }}"
									...
									<div class="tab-content mt-3">
										...
										<div class="form-group row">
											...
											<div class="col-md-2">
												<input type="text" class="form-control NO_BUKTI" id="NO_BUKTI" name="NO_BUKTI" placeholder="Masukkan Bukti#"
										...
										<div class="form-group row">
											...
											<div class="col-md-3">
												...
												<input type="text" hidden class="form-control NAMAS" id="NAMAS" name="NAMAS" value="{{ $header->NAMAS }}"
													placeholder="Masukkan Nama">
												<input type="text" hidden class="form-control HARI" id="HARI" name="HARI" value="{{ $header->HARI }}"
													placeholder="Masukkan Hari">

											</div>

											<div class="col-md-3">
												<input type="checkbox" class="form-check-input" id="PKP" name="PKP" readonly value="{{ $header->PKP }}"
													{{ $header->PKP == 1 ? 'checked' : '' }}>
										...
										<div class="form-group row">
											...
											<div class="col-md-5 form-group row special-input-label">
												...
												<input type="text" class="NOTES" id="NOTES" name="NOTES" value="{{ $header->NOTES }}" placeholder=" ">
											...
											<table id="datatable" class="table-striped table-border table">
												<thead>
													<tr>
														...
														<th width="75px" style="text-align: center;">Qty</th>
												...
												<tbody>
													...
													@foreach ($detail as $detail)
														<tr>
															...
															<td>
																<input name="KD_BRG[]" id="KD_BRG{{ $no }}" type="text" class="form-control KD_BRG" value="{{ $detail->KD_BRG }}"
															...
															<td>
																<input name="QTY[]" onclick="select()" onblur="hitung()" value="{{ $detail->QTY }}" id="QTY{{ $no }}"
																	type="text" style="text-align: right" class="form-control QTY text-primary">
															</td>

															<td hidden>
																<input name="HARGA[]" onclick="select()" onblur="hitung()" value="{{ $detail->HARGA }}" id="HARGA{{ $no }}"
																	type="text" style="text-align: right" class="form-control HARGA">
															</td>

															<td hidden>
																<input name="TOTAL[]" onblur="hitung()" value="{{ $detail->TOTAL }}" id="TOTAL{{ $no }}" type="text"
																	style="text-align: right" class="form-control TOTAL" readonly>
															</td>
															<td hidden>
																<input name="PPNX[]" onblur="hitung()" value="{{ $detail->PPN }}" id="PPNX{{ $no }}" type="text"
																	style="text-align: right" class="form-control PPNX" readonly>
															</td>
															<td hidden>
																<input name="DPP[]" onblur="hitung()" value="{{ $detail->DPP }}" id="DPP{{ $no }}" type="text"
																	style="text-align: right" class="form-control DPP" readonly>
															</td>
												...
												<tfoot>
													...
													<td></td>
													<!-- <td></td> -->
													<td><input class="form-control TTOTAL_QTY text-primary font-weight-bold" style="text-align: right" id="TTOTAL_QTY" name="TTOTAL_QTY"
									...
									<div class="col-md-12 form-group row mt-3">
										<div class="col-md-4">
											<button hidden type="button" id='TOPX' onclick="location.href='{{ url('/hdh/edit/?idx=' . $idx . '&tipx=top&flagz=' . $flagz . '') }}'"
												class="btn btn-outline-primary">Top</button>
											<button hidden type="button" id='PREVX'
												onclick="location.href='{{ url('/hdh/edit/?idx=' . $header->NO_ID . '&tipx=prev&flagz=' . $flagz . '&buktix=' . $header->NO_BUKTI) }}'"
												class="btn btn-outline-primary">Prev</button>
											<button hidden type="button" id='NEXTX'
												onclick="location.href='{{ url('/hdh/edit/?idx=' . $header->NO_ID . '&tipx=next&flagz=' . $flagz . '&buktix=' . $header->NO_BUKTI) }}'"
												class="btn btn-outline-primary">Next</button>
											<button hidden type="button" id='BOTTOMX'
												onclick="location.href='{{ url('/hdh/edit/?idx=' . $idx . '&tipx=bottom&flagz=' . $flagz . '') }}'"
												class="btn btn-outline-primary">Bottom</button>
										</div>
										<div class="col-md-5">
											<button hidden type="button" id='NEWX' onclick="location.href='{{ url('/hdh/edit/?idx=0&tipx=new&flagz=' . $flagz . '') }}'"
											...
											<button hidden type="button" id='UNDOX'
												onclick="location.href='{{ url('/hdh/edit/?idx=' . $idx . '&tipx=undo&flagz=' . $flagz . '') }}'" class="btn btn-info">Undo</button>
										...
										<div class="col-md-3">
											...
											<!-- <button type="button" id='CLOSEX'  onclick="location.href='{{ url('/hdh?flagz=' . $flagz . '') }}'" class="btn btn-outline-secondary">Close</button> -->
...
@section('footer-scripts')
	...
	<script>
		...
		$(document).ready(function() {
						...
						if (resp.length > 1) {
							...
							for (i = 0; i < resp.length; i++) {
								...
								dTableBBarang.row.add([
									'<a href="javascript:void(0);" onclick="chooseBarang(\'' + resp[i].KD_BRG + '\',\'' + resp[i]
									.KD_BRG2 + '\', \'' + resp[i].NA_BRG + '\' , \'' + resp[i].SATUAN + '\' )">' + resp[i]
									.KD_BRG + '</a>', resp[i].KD_BRG2,
									resp[i].NA_BRG,
									resp[i].SATUAN,
								]);
							}
							...
							$("#KD_BRG2" + rowidBarang).val(resp[0].KD_BRG2);
			...
			chooseBarang = function(KD_BRG, KD_BRG2, NA_BRG, SATUAN) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					Swal.fire({
						...
						loc = "{{ url('/hdh/delete/' . $header->NO_ID) }}" + '?flagz=' + encodeURIComponent(flagz);
		...
		function closeTrans() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					loc = "{{ url('/hdh/') }}" + '?flagz=' + encodeURIComponent(flagz);
					window.location = loc;
				} else {
					Swal.fire({
						icon: 'info',
		...
		function CariBukti() {
			...
			var flagz = "{{ $flagz }}";
			var cari = $("#CARI").val();
			var loc = "{{ url('/hdh/edit/') }}" + '?idx={{ $header->NO_ID }}&tipx=search&flagz=' + encodeURIComponent(flagz) + '&buktix=' +
