...
<div class="content-wrapper">
        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/kmterima/store?flagz='.$flagz.'') : url('/kmterima/update/'.$header->NO_ID.'&flagz='.$flagz.'' ) }}" method="POST" name ="entri" id="entri" >
                        ...
                        <div class="tab-content mt-3">
							...
							<div class="form-group row">
								...
								<div class="col-md-1" align="right">
									<label for="CBG_TUJU" class="form-label">Penerima</label>
								</div>
								<div class="col-md-1">
                            ...
                            <table id="datatable" class="table table-striped table-border">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="150px" style="text-align: center;">
                                           ...
                                           <label for="NO_TERIMA" class="form-label">No. Terima</label></th>
                                        <th width="550px" style="text-align: center;">Pengirim</th>
								...
								@foreach ($detail as $detail)
                                    <tr>
										...
										<td>
                                            <input name="NO_TERIMA[]" id="NO_TERIMA{{$no}}" type="text" class="form-control NO_TERIMA "
											value="{{$detail->NO_TERIMA}}" onblur="browseTerima({{$no}})">
                                        </td>

										<td>
                                            <input name="CBG_TERIMA[]" id="CBG_TERIMA{{$no}}" type="text" class="form-control CBG_TERIMA " value="{{$detail->CBG_TERIMA}}" readonly required>
                                        </td>
										...
										<td hidden><input name="QTY[]" onclick="select()" onkeyup="hitung()" value="{{$detail->QTY}}" id="QTY{{$no}}" type="text" style="text-align: right"  class="form-control QTY text-primary" required></td>
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button hidden type="button" id='TOPX'  onclick="location.href='{{url('/kmterima/edit/?idx=' .$idx. '&tipx=top&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Top</button>
								<button hidden type="button" id='PREVX' onclick="location.href='{{url('/kmterima/edit/?idx='.$header->NO_ID.'&tipx=prev&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Prev</button>
								<button hidden type="button" id='NEXTX' onclick="location.href='{{url('/kmterima/edit/?idx='.$header->NO_ID.'&tipx=next&flagz='.$flagz.'&buktix='.$header->NO_BUKTI )}}'" class="btn btn-outline-primary">Next</button>
								<button hidden type="button" id='BOTTOMX' onclick="location.href='{{url('/kmterima/edit/?idx=' .$idx. '&tipx=bottom&flagz='.$flagz.'' )}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
								<button hidden type="button" id='NEWX' onclick="location.href='{{url('/kmterima/edit/?idx=0&tipx=new&flagz='.$flagz.'' )}}'" class="btn btn-warning">New</button>
								<button hidden type="button" id='EDITX' onclick='hidup()' class="btn btn-secondary">Edit</button>
								<button hidden type="button" id='UNDOX' onclick="location.href='{{url('/kmterima/edit/?idx=' .$idx. '&tipx=undo&flagz='.$flagz.'' )}}'" class="btn btn-info">Undo</button>
							...
							<div class="col-md-3">
								...
								<!-- <button type="button" id='CLOSEX'  onclick="location.href='{{url('/kmterima?flagz='.$flagz.'' )}}'" class="btn btn-outline-secondary">Close</button> -->
	...
	<div class="modal fade" id="browseTerimaModal" tabindex="-1" role="dialog" aria-labelledby="browseTerimaModalLabel" aria-hidden="true">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title" id="browseTerimaModalLabel">Cari Item</h5>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
			</button>
		  </div>
		  <div class="modal-body">
			<table class="table table-stripped table-bordered" id="table-bTerima">
				<thead>
					<tr>
						{{-- <th>Item#</th> --}}
                        <th>No. Terima</th>
...
<script>
    ...
    baris=<?=$no?>;
		...
		var dTableBTerima;
		var rowidTerima;
		loadDataBTerima = function(){

			$.ajax(
			{
				type: 'GET',
				url: "{{url('terima/browse_kum')}}",
				...
				async : false,
				data: {
						'NO_TERIMA': $("#NO_TERIMA"+rowidTerima).val(),
...

				{
					...
					if ( resp.length > 1 )
					{
							...
							for(i=0; i<resp.length; i++){
								...
								dTableBTerima.row.add([
									'<a href="javascript:void(0);" onclick="chooseTerima(\''+resp[i].NO_TERIMA+'\', \''+resp[i].CBG_TERIMA+'\' )">'+resp[i].NO_TERIMA+'</a>',
                                    resp[i].CBG_TERIMA,
								]);
							}
							dTableBTerima.draw();
					...
					{
						$("#NO_TERIMA"+rowidTerima).val(resp[0].NO_TERIMA);
                        $("#CBG_TERIMA"+rowidTerima).val(resp[0].CBG_TERIMA);
					}
				}
			});
			$("#LOADX").hide();
		...
		dTableBTerima = $("#table-bterima").DataTable({

		});

		browseTerima = function(rid){
			rowidTerima = rid;
			$("#NO_TERIMA"+rowidTerima).val("");
			loadDataBTerima();


			if ( $("#NO_TERIMA"+rowidTerima).val() == '' ) {
					$("#browseTerimaModal").modal("show");
			}
		}

		chooseTerima = function(NO_TERIMA, CBG_TERIMA){
			$("#NO_TERIMA"+rowidTerima).val(NO_TERIMA);
            $("#CBG_TERIMA"+rowidTerima).val(CBG_TERIMA);
			$("#browseTerimaModal").modal("hide");
		}
...
///////////////////////////////////////
	...
	// 	{
	// 		window.location ="{{url('/hkirim/delete/'.$header->NO_ID .'/?flagz='.$flagz.'' )}}";
	// 		//return true;
	// 	}
	// 	return false;
	// }
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
	            	...
	            	loc = "{{ url('/kmterima/delete/'.$header->NO_ID) }}" + '?flagz=' + encodeURIComponent(flagz) ;
	...
	function closeTrans() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
	        	loc = "{{ url('/kmterima/') }}" + '?flagz=' + encodeURIComponent(flagz) ;
				window.location = loc ;
			} else {
				Swal.fire({
					icon: 'info',
	...
	function CariBukti() {
		...
		var flagz = "{{ $flagz }}";
		var cari = $("#CARI").val();
		var loc = "{{ url('/kmterima/edit/') }}" + '?idx={{ $header->NO_ID}}&tipx=search&flagz=' + encodeURIComponent(flagz) + '&buktix=' +encodeURIComponent(cari);
		window.location = loc;
    ...
    function tambah() {
		...
		html=`<tr>
                ...
                <td>
				    <input name='NO_TERIMA[]' data-rowid=${idrow} onblur='browseTerima(${idrow})' id='NO_TERIMA${idrow}' type='text' class='form-control  NO_TERIMA' >
                </td>
                <td>
				    <input name='CBG_TERIMA[]'   id='CBG_TERIMA${idrow}' type='text' class='form-control  CBG_TERIMA' readonly required>
                </td>
