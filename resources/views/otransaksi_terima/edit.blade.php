                    ...
                    <form action="{{($tipx=='new')? url('/terima/store?flagz='.$flagz.'') : url('/terima/update/'.$header->NO_ID.'&flagz='.$flagz.'' ) }}" method="POST" name ="entri" id="entri" >
                        ...
                        <div class="tab-content mt-3">
							...
							<div class="form-group row">
								...
								<div class="col-md-2 input-group" >
									<input type="text" class="form-control BARCODE" id="BARCODE" name="BARCODE" placeholder="Pilih No. Expedisi"value="{{$header->NO_KKIRIM}}" style="text-align: left">
									{{-- <button type="button" class="btn btn-primary" onclick="browseKkirim()"><i class="fa fa-search"></i></button> --}}
								</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-2 input-group" >
									<input type="text" class="form-control NO_KIRIM" id="NO_KIRIM" name="NO_KIRIM" placeholder="Pilih Bukti Kirim"value="{{$header->NO_KIRIM}}" style="text-align: left" readonly >
                            ...
                            <table id="datatable" class="table table-striped table-border">
								...
								@foreach ($detail as $detail)		
                                    <tr>
										...
										<td>
											<input name="QTY_KIRIM[]" onclick="select()" onkeyup="hitung()" value="{{$detail->QTY_KIRIM}}" id="QTY_KIRIM{{$no}}" type="text" style="text-align: right"  class="form-control QTY_KIRIM text-primary" readonly required>
										</td>
						...
						<div class="mt-3 col-md-12 form-group row">
							...
							<div class="col-md-3">
								...
								<!-- <button type="button" id='CLOSEX'  onclick="location.href='{{url('/terima?flagz='.$flagz.'' )}}'" class="btn btn-outline-secondary">Close</button> -->
...
//////////////////////////////////////////////////////////////////
		$("#BARCODE").keydown(function(e){
			...
			if(e.keyCode == 13){
				e.preventDefault();
				getKirimd($("#BARCODE").val(), 'BARCODE');
			}
		});
		//		CHOOSE KKirim
		// var dTableBKkirim;
		// loadDataBKkirim = function(){
		
		// 	$.ajax(
		// 	{
		// 		type: 'GET', 		
		// 		url: '{{url('kmkirim/browse_terikum')}}',
		// 		data: {
		// 			BARCODE: $("#BARCODE").val(),
		// 		},
		...
		// 				dTableBKkirim.row.add([
		// 					'<a href="javascript:void(0);" onclick="chooseKkirim(\''+resp[i].NO_KKIRIM+'\', \''+resp[i].CBG+'\' )">'+resp[i].NO_KKIRIM+'</a>',
		// 					resp[i].CBG,
		// 				]);
		// 			}
		// 			dTableBKkirim.draw();
		...
		// dTableBKkirim = $("#table-bkkirim").DataTable({
			
		// });
		
		// browseKkirim = function(){
		// 	loadDataBKkirim();
		// 	$("#browseKkirimModal").modal("show");
		// }
		
		// chooseKkirim = function( NO_KKIRIM, CBG){

		// 	$("#BARCODE").val(NO_KKIRIM);
		// 	$("#CBG_DARI").val(CBG);		
		// 	$("#browseKkirimModal").modal("hide");
			
		// 	getKirimd(NO_KKIRIM, 'BARCODE');
		// }
				...
				success: function( resp )
				{
					...
					for(i=0; i<resp.length; i++){
						html+=`<tr>
									...
									<td>
										<input name='QTY_KIRIM[]' onclick='select()' onblur='hitung()' id='QTY_KIRIM${i}' value="${resp[i].QTY_KIRIM}" type='text' style='text-align: right' class='form-control QTY_KIRIM text-primary' readonly >
									</td>
									<td>
...
function ambil_barang()
	...
	function cekDetail2(){
		...
		$(".QTY").each(function() {
			...
			let z = $(this).closest('tr');
			var QTY1X = parseFloat(z.find('.QTY_KIRIM').val().replace(/,/g, ''));
