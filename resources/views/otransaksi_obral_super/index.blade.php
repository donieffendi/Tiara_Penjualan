                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-md-6">
                                        <a href="{{ route($menuType == 'FS' ? 'tentryflashsale.edit' : 'tobralsupermarket.edit', ['status' => 'simpan']) }}"
...
@section('javascripts')
    ...
    <script>
        ...
        function editData(noBukti) {
            ...
            window.location.href = "{{ url('/') }}/" + (menuType == 'FS' ? 'tentryflashsale' : 'tobralsupermarket') +
                "/edit?status=edit&no_bukti=" +
                noBukti;
        }
        ...
        function deleteData(noBukti) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('/') }}/" + (menuType == 'FS' ? 'tentryflashsale' :
                            'tobralsupermarket') + "/delete/" + noBukti,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
        ...
        function printData(noBukti) {
            var menuType = '{{ $menuType }}';
            var url = "{{ url('/') }}/" + (menuType == 'FS' ? 'tentryflashsale' : 'tobralsupermarket') + "/print";
