...
class RKartupController extends Controller
{
    public function kartu()
    {
        ...
        return view('oreport_piu.kartu')->with(['cbg' => $cbg])->with(['kodec' => $cust])->with(['hasil' => []]);
    }
	...
	public function getPiuKartu(Request $request)
    {
		...
		(
			...
			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d/%m/%Y') AS TGL, KODEC, NAMAC, PER$bulan AS TOTAL, 0 AS BAYAR,
					MONTH(TGL) AS urut, $tahun AS `year`
			from jualx where YER='$tahun' $cust $filtergol union all

			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d/%m/%Y') AS TGL, KODEC, NAMAC, 0 AS TOTAL, BAYAR,
					MONTH(TGL) AS urut, $tahun AS `year`  
			from piu where PER='$periode' $cust $filtergol

			group by KODEC order by KODEC
...
/* 			$filterlebih30=" ";
	...
	public function jasperPiuKartu(Request $request) 
	{
		...
		(
			...
			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d-%m-%Y') AS TGL, KODEC, NAMAC, PER$bulan AS TOTAL, 0 AS BAYAR 
			from jualx where YER='$tahun' and PER$bulan<>0 $cust $filtercbg union all

			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d-%m-%Y') AS TGL, KODEC, NAMAC, 0 AS TOTAL, BAYAR  
			from piu where PER='$periode' $cust $filtercbg
		)  as kartup;
		");
		...
		{
			$cbg = Cbg::groupBy('CBG')->get();
			return view('oreport_piu.kartu')->with(['cbg' => $cbg])->with(['hasil' => $query]);
		}
