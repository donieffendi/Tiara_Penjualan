...
class RKmkirimController extends Controller
{
  	...
  	public function report()
    {
        ...
        return view('oreport_kirim.report')->with(['kodec' => $kodec])->with(['cbg' => $cbg])->with(['hasil' => []]);
    }
	  

	public function jasperKmkirimReport(Request $request) 
	{
		$file 	= 'ambiln';
		...
		$query = DB::SELECT("
			...
			SELECT a.*, b.*, DATE_FORMAT(a.TGL, '%d-%m-%Y') AS TGL
			FROM kirim a, kirimd b
		...
		if($request->has('filter'))
		{
			...
			return view('oreport_kirim.report')->with(['cbg' => $cbg])->with(['hasil' => $query]);
		}
