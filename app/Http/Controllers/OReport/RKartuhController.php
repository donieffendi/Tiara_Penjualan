...
class RKartuhController extends Controller
{
    ...
    public function kartu()
    {
        ...
        return view('oreport_hut.kartu')->with(['cbg' => $cbg])->with(['kodes' => $sup])->with(['hasil' => []]);
    }
	...
	public function getHutKartu(Request $request)
    {
		...
		(
			...
			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d/%m/%Y') AS TGL, KODES, NAMAS, PER$bulan AS TOTAL, 0 AS BAYAR 
			from belix where belix.YER='$tahun' $filterbelix union all

			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d/%m/%Y') AS TGL, KODES, NAMAS, 0 AS TOTAL, BAYAR  
	...
	public function sisa()
    {
        ...
        return view('oreport_hut.sisa')->with(['kodes' => $sup])->with(['cbg' => $cbg])->with(['hasil' => []]);
    }
	...
	public function jasperHutKartu(Request $request) 
	{
		...
		(
			...
			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d-%m-%Y') AS TGL, KODES, NAMAS, PER$bulan AS TOTAL, 0 AS BAYAR,
					MONTH(TGL) AS urut, $tahun AS `year`,PER$bulan AS MASUK,0 AS KELUAR,0 AS LAIN
			from belix 
			where belix.YER='$tahun' and PER$bulan<>0 $filterbelix $filtercbg union all

			SELECT NO_BUKTI, DATE_FORMAT(TGL, '%d-%m-%Y') AS TGL, KODES, NAMAS, 0 AS TOTAL, BAYAR, 
					MONTH(TGL) AS urut, $tahun AS `year`,0 AS MASUK,BAYAR AS KELUAR,0 AS LAIN  
			from hut where PER='$periode' $sup $filtercbg

			group by KODES order by KODES
		) as  kartuh ;
		");
		...
		if($request->has('filter'))
		{
			...
			$cbg = Compan::groupBy('EXT')->get();
			return view('oreport_hut.kartu')->with(['cbg' => $cbg])->with(['hasil' => $query]);
		}
