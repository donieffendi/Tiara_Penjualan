...
class RLakuPerHariController extends Controller
{
    ...
    private function getLakuPerHariData($cbg, $hari)
    {
        try {
            ...
            // Build union SQL (pastikan passing $hari)
            $unionSql = $this->buildUnionQuery($cbg, $bulanData, $hari);

            // Main query (tidak memakai SET @...; semua nilai sudah di-interpolate)
            $query = "
                ...
                INNER JOIN {$cbg}.brgdt AS BRGDT
                    ON OLE.KD_BRG = BRGDT.KD_BRG
                AND BRGDT.CBG = '{$cbg}'
                ORDER BY OLE.KD_BRG ASC
            ";

            // Execute raw SQL (no bindings necessary here)
            ...
            foreach ($hasilData as $item) {
                $result[] = [
                    ...
                    'LHUSUL' => floatval($item->LHUSUL),
                    'DTRMAX' => floatval($item->DTRMAX),
                    'USULANINI' => floatval($item->USULANINI),
    ...
    /**
     * @param string $cbg
     * @param array $bulanData ['skrg','satu','dua'] (2-digit strings)
     * @param int $hari
     * @return string SQL fragment (UNION ALL)
     */
    private function buildUnionQuery($cbg, $bulanData, $hari)
    {
        // simple sanitization for schema/table usage: only allow alnum & underscore
        if (!preg_match('/^[A-Za-z0-9_]+$/', $cbg)) {
            throw new \Exception('Nama cabang tidak valid untuk digunakan sebagai schema.');
        }
        ...
        // Note: use direct interpolation for $hari and $cbg (sanitized above)
        $sql = "
