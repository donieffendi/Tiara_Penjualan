...
class JackfileController extends Controller
{

    public function index() {
        return view('olain_jackfile.index');
    }

    public function getJackfile(Request $request)
    {
    
        $jackfile = DB::SELECT("SELECT * from jackfile");
        
        return Datatables::of($jackfile)
                ->addIndexColumn()
                ->addColumn('action', function($row) {
					if (Auth::user()->divisi=="programmer" || Auth::user()->divisi=="owner" || Auth::user()->divisi=="sales")
					{   
                        // url untuk delete di index
                        $url = "'".url("jackfile/delete/" . $row->NO_ID )."'";
                        // batas
                        ...
                        $btnPrivilege =
                            '
                                    <a class="dropdown-item" href="jackfile/edit/?idx=' . $row->NO_ID . '&tipx=edit";
                                    <i class="fas fa-edit"></i>
                                        Edit
                                    </a>
                                    <a hidden class="dropdown-item btn btn-danger" href="jackfile/cetak/' . $row->NO_ID . '">
                    ...
                    $actionBtn =
                        ...
                        <div class="dropdown show" style="text-align: center">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                <a hidden class="dropdown-item" href="jackfile/show/' . $row->NO_ID . '">
    ...
    public function edit(Request $request , Jackfile $jackfile)
    {
		...
		if ($tipx=='search') {
		   ...
		   $bingco = DB::SELECT("SELECT NO_ID, kodeh from jackfile
		                 where kodeh = '$kodex'						 
		...
		//   $kd_brg = $brg->KD_BRG; 
	  	...
	  	if ( $idx != 0 ) 
		{
			$jackfile = Jackfile::where('NO_ID', $idx )->first();	
	    }
		else
		{
            $jackfile = new Jackfile;			 
		}
        ...
        return view('olain_jackfile.edit', $data)->with(['tipx' => $tipx, 'idx' => $idx ]);		
    }
    ...
    public function update(Request $request, Jackfile $jackfile)
    {
        ...
        $jackfile->update(
            [
                'J_MIN'           => (float) str_replace(',', '', $request['J_MIN']),
                'J_JACK'          => (float) str_replace(',', '', $request['J_JACK']),
                'J_UTAMA'         => (float) str_replace(',', '', $request['J_UTAMA']),
                'J_SATU'          => (float) str_replace(',', '', $request['J_SATU']),
                'J_DUA'           => (float) str_replace(',', '', $request['J_DUA']),
                'J_HIBUR'         => (float) str_replace(',', '', $request['J_HIBUR']),
                'JARAK'           => (float) str_replace(',', '', $request['JARAK']),
                'J_BELANJA'       => (float) str_replace(',', '', $request['J_BELANJA']),
                'J_PERSEN'        => (float) str_replace(',', '', $request['J_PERSEN']),
                'J_JAM'           => date('H:i', strtotime($request['J_JAM'])),
                'JAM_AK'          => date('H:i', strtotime($request['JAM_AK'])),
                'TGL_AW'          => date('Y-m-d', strtotime($request['TGL_AW'])),
                'TGL_AK'          => date('Y-m-d', strtotime($request['TGL_AK'])),
                'J_PILIH'         => ($request['J_PILIH'] == null) ? "" : $request['J_PILIH'],
                'JUMJACK'         => (float) str_replace(',', '', $request['JUMJACK']),
                'aktv'            => (float) str_replace(',', '', $request['aktv']),
