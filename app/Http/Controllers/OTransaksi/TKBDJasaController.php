...
class TKBDJasaController extends Controller
{
    /**
    ...
    {
        return view('otransaksi_KBDJasa.index');
    }
    ...
    /**
     * Browse departments with search functionality
     */
    ...
    /**
     * Browse customers with search functionality (not used in Delphi but keeping for compatibility)
     */
    ...
    /**
     * Get KBD Jasa data for DataTables (from Delphi tampil procedure)
     */
    public function getKBDJasa()
    {
        $periode = session('periode');
        $flag = session('flag');

        // Based on Delphi tampil procedure
        $kbdJasa = DB::SELECT("SELECT no_bukti, na_brg, qty, satuan, ukuran, harga, cbg, kd_dept, per,
                                total, batas1, tg_pbl, akunt, ket, tgl, dept, usrnm,
                                (SELECT CASE WHEN posted = 1 THEN 1 ELSE 0 END) as posted
                                FROM tpondg
                                WHERE PER = '$periode' AND CBG = '$flag'
                                GROUP BY no_bukti
                                ORDER BY no_bukti ASC");

        return Datatables::of($kbdJasa)
            ->addIndexColumn()
            ...
            ->addColumn('action', function ($row) {
                if (
                    ...
                    if ($row->posted == 0 && $currentPeriod == $row->per) {
                        $btnEdit = '<a class="dropdown-item" href="TKBDJasa/edit?idx=' . $row->no_bukti . '&tipx=edit">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>';

                        $url = "'" . url("TKBDJasa/delete/" . $row->no_bukti) . "'";
    ...
    public function edit(Request $request)
    {
        ...
        if ($tipx == 'prev' && !empty($idx)) {
            $bingco = DB::SELECT("SELECT no_bukti from tpondg
                                 WHERE no_bukti < '$idx' AND PER = '$periode'
                                 GROUP BY no_bukti
        ...
        if (!empty($idx)) {
            $header = DB::SELECT("SELECT no_bukti, tgl, notes, kd_dept, dept, usrnm,
                                    SUM(total) as total
                                    FROM tpondg
                                    WHERE no_bukti = '$idx'
                                    GROUP BY no_bukti")[0];
        ...
        return view('otransaksi_KBDJasa.edit', $data)->with(['tipx' => $tipx, 'idx' => $idx]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request['NO_BUKTI'] == '+' || empty($request['NO_BUKTI'])) {
                ...
                DB::table('tpondg')->where('no_bukti', $noBukti)->delete();
            }

            // Insert detail records (from Delphi logic)
    ...
    public function destroy($noBukti)
    {
        ...
        try {
            ...
            $record = DB::SELECT("SELECT COUNT(*) as count,
                                   ...
                                   FROM tpondg WHERE no_bukti = ?", [$noBukti])[0];
            ...
            $currentPeriod = DB::SELECT("SELECT CONCAT(LPAD(MONTH(NOW()),2,0),'/',YEAR(NOW())) as periy")[0]->periy;
            $dataPeriod = DB::SELECT("SELECT per FROM tpondg WHERE no_bukti = ? LIMIT 1", [$noBukti])[0]->per;

            if ($currentPeriod != $dataPeriod) {
                return response()->json(['error' => 'Hanya dapat menghapus data periode berjalan'], 400);
            }

            // Delete all records with this no_bukti
            DB::table('tpondg')->where('no_bukti', $noBukti)->delete();

            DB::commit();

            return redirect('/TKBDJasa')->with('status', 'Data berhasil dihapus');
        } catch (\Exception $e) {
            DB::rollBack();
