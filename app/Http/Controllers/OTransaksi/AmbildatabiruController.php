...
class AmbildatabiruController extends Controller
{
	use Terbilang;
	
    public function index()
    {
        return view('otransaksi_ambildatabiru.edit');
    }
    ...
    public function store(Request $request)
    {
        ...
        if($CBG == 'Z'){
            $header = DB::connection('tgz')->table('beli')->where('NO_BUKTI', $NO_BUKTI)->get();
            $detail = DB::connection('tgz')->table('belid')->where('NO_BUKTI', $NO_BUKTI)->get();
        }elseif($CBG == 'M'){
            $header = DB::connection('tmm')->table('beli')->where('NO_BUKTI', $NO_BUKTI)->get();
            $detail = DB::connection('tmm')->table('belid')->where('NO_BUKTI', $NO_BUKTI)->get();
        }elseif($CBG == 'S'){
            $header = DB::connection('sop')->table('beli')->where('NO_BUKTI', $NO_BUKTI)->get();
            $detail = DB::connection('sop')->table('belid')->where('NO_BUKTI', $NO_BUKTI)->get();
        };

        foreach ($header as $row) {
            DB::connection('mysql')->table('beli')->insert([
        ...
        foreach ($detail as $row) {
            DB::connection('mysql')->table('belid')->insert([
                'NO_BUKTI' => $row->NO_BUKTI,  
                'KD_BRG'   => $row->KD_BRGH,   
                'NA_BRG'   => $row->NA_BRGH,  
                'FLAG'     => $row->FLAG,     
        ...
        return redirect('/ambildatabiru')->with('status', 'Data baru berhasil diambil');
    }
}
