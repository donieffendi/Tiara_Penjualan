...
class PostedController extends Controller
{
    ...
    {
        return view('otransaksi_posted.index');
    }

    // public function index_posting(Request $request)
    public function index_posting()
    {   
        return view('otransaksi_posted.post');
    }
    ...
    public function getPosted(Request $request)
    {
        ...
        $posted = DB::SELECT("SELECT * 
                    from masuk  
                    where POSTED = '0'
                    ORDER BY NO_BUKTI ");
       
        return Datatables::of($posted)
            ->addIndexColumn()
            ->addColumn('action', function ($row) {
                if (Auth::user()->divisi=="programmer" || Auth::user()->divisi=="owner" || Auth::user()->divisi=="assistant" || Auth::user()->divisi=="penjualan") 
                {
                    $btnEdit = ($row->POSTED == 1) ? ' onclick= "alert(\'Transaksi ' . $row->NO_BUKTI . ' sudah diposting!\')" href="#" ' : ' href="posted/edit/' . $row->NO_ID . '" ';
                    $btnDelete = ($row->POSTED == 1) ? ' onclick= "alert(\'Transaksi ' . $row->NO_BUKTI . ' sudah diposting!\')" href="#" ' : ' href="posted/delete/' . $row->NO_ID . '" ';
                    ...
                    $btnPrivilege ='
                                ...
                                <a class="dropdown-item btn btn-danger" href="posted/print/' . $row->NO_ID . '">
                ...
                $actionBtn =
                    ...
                    <div class="dropdown show" style="text-align: center">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                            <a class="dropdown-item" href="posted/show/' . $row->NO_ID . '">
    ...
    {   
        ...
        if ($request->session()->has('posttimer'))
        {
            if ( (time() - $request->session()->get('posttimer')) <= 5)
            {
                ...
                return redirect('/posted/index-posting')->with('gagal', 'Terlalu cepat. Ulangi 5 detik lagi..');
            }
        }
        ...
        if ($CEK) {
            foreach ($CEK as $key => $value) {
                ...
                DB::SELECT("UPDATE masuk SET POSTED = '1'
                            where NO_ID =" . $CEK[$key] . ";");
        ...
        if($hasil!='')
        {
            return redirect('/posted/index-posting')->with('status', 'Proses Posted Request..')->with('gagal', $hasil);
        }
        else
        {
            return redirect('/posted/index-posting')->with('status', 'Posted selesai..');
        }
    ...
    public function cetak(Posted $posted, Request $request)
    {

		$jenisx = $posted->JENIS;
		$no_bukti = $posted->NO_CHBG;
		$file ='';
		...
		$terbilangx = ucwords($this->pembilang($posted->TOTAL));
