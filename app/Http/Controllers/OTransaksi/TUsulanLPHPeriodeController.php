...
class TUsulanLPHPeriodeController extends Controller
{
    ...
    private function prosesNew($request, $CBG, $username, $periode)
    {
        try {
            ...
            $queryCheck = "SELECT LPH{$CBG} AS LPH FROM PERID WHERE KD_PERI = ?";

            $check = DB::select($queryCheck, [$periode]);
            ...
            DB::statement("CALL lph_1periode(?, ?, ?)", [$CBG, $periode, $username]);
            DB::statement("CALL lph_2periode(?, ?, ?)", [$CBG, $periode, $username]);
    ...
    private function prosesPosting($request, $CBG, $username, $periode)
    {
        ...
        foreach ($items as $item) {
            ...
            $check      = DB::select($queryCheck, [$perx, $subx, $ket]);
            ...
            foreach ($cabangs as $cab) {
                ...
                try {
                    DB::statement($updateBrg, [$cbgx, $username, $perx, $subx, $ket]);
            ...
            DB::statement($updateBrgdt, [$cbgx, $username, $perx, $ket, $subx]);
            Log::info('TUsulanLPHPeriode prosesPosting: Updated brgdt in ' . $cbgx);

            // Call repair procedure - use cbgx connection
            DB::statement("CALL dcts_repair_lph_periode(?, ?, ?, ?)", [$perx, $subx, $ket, $username]);
            ...
            DB::statement($insertHistory, [$subx, $cbgx, $perx, $ket]);
            ...
            DB::statement($delete, [$subx, $cbgx, $perx, $ket]);
    ...
    private function updateLPH($request, $CBG, $username)
    {
        ...
        $result    = DB::select($queryCalc, [$kd_brg, $lph, $dtr]);
    ...
    public function print(Request $request)
    {
        ...
        $query = DB::select("
        SELECT *, DATE_FORMAT(TG_SMP,'%d/%m/%Y') TGL FROM oleolang
        WHERE sub = ? AND per = ? AND ket = ?

        UNION ALL

        SELECT *, DATE_FORMAT(TG_SMP,'%d/%m/%Y') TGL FROM oleolang_his
        WHERE sub = ? AND per = ? AND ket = ?

        ORDER BY kd_brg
    ", [$sub, $per, $ket, $sub, $per, $ket]);

        $file = 'usulan_lh_periode';
        ...
        ob_end_clean();
        return $PHPJasperXML->outpage("I"); // tampilkan PDF
    }
