...
class SupFCController extends Controller
{

    public function index()
    {
        return view('master_suplier_food_center.index');
    }
    ...
    public function show(Request $request)
    {
        ...
        if ($tipx == 'top') {
            $bingco = DB::SELECT("SELECT NO_ID, KODES from sup ORDER BY KODES ASC LIMIT 1");
            if (!empty($bingco)) {
                $idx = $bingco[0]->NO_ID;
            } else {
                $idx = 0;
        ...
        if ($tipx == 'prev') {
            $kodex = $request->kodex;
            $bingco = DB::SELECT("SELECT NO_ID, KODES from sup where KODES < '$kodex' ORDER BY KODES DESC LIMIT 1");
            if (!empty($bingco)) {
                $idx = $bingco[0]->NO_ID;
            }
            // jika tidak ada data sebelumnya, tetap di posisi saat ini (idx tidak berubah)
        }

        if ($tipx == 'next') {
            $kodex = $request->kodex;
            $bingco = DB::SELECT("SELECT NO_ID, KODES from sup where KODES > '$kodex' ORDER BY KODES ASC LIMIT 1");
            if (!empty($bingco)) {
                $idx = $bingco[0]->NO_ID;
            }
            // jika tidak ada data berikutnya, tetap di posisi saat ini (idx tidak berubah)
        }

        if ($tipx == 'bottom') {
            $bingco = DB::SELECT("SELECT NO_ID, KODES from sup ORDER BY KODES DESC LIMIT 1");
            if (!empty($bingco)) {
                $idx = $bingco[0]->NO_ID;
            } else {
                $idx = 0;
        ...
        if ($idx != 0) {
            $header = Sup::where('NO_ID', $idx)->first();
        } else {
            $header = new Sup;
        }

        return view('master_suplier_food_center.show')->with(['tipx' => $tipx, 'idx' => $idx, 'header' => $header]);
    }

    public function getSupFC(Request $request)
    {
        ...
        return Datatables::of($query)
            ...
            ->addColumn('action', function ($row) {
                if (Auth::user()->divisi == "programmer" || Auth::user()->divisi == "owner" || Auth::user()->divisi == "sales") {
                    ...
                    $url = "'" . url("sup-food-center/delete/" . $row->NO_ID) . "'";
                    // batas
                    ...
                    $btnPrivilege =
                        '
                                    <a hidden class="dropdown-item" href="sup-food-center/edit/?idx=' . $row->NO_ID . '&tipx=edit";
                    ...
                    '
                        <div class="dropdown show" style="text-align: center">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                <a class="dropdown-item" href="sup-food-center/show/?idx=' . $row->NO_ID . '&tipx=top">
