...
class ImportSqlController extends Controller
{
    ...
    public function index() {
        $jns = DB::SELECT("SELECT KODE from dataubahno where jns in ('BRGCH','SUPCH','SUPD2')");
        return view('master_pengesahan_import_sql.index')->with('jns', $jns);
    }

    public function getDataUbahno($kode)
    {
        $data = DB::table('dataubahno')->where('KODE', $kode)->first();
    ...
    public function getImportSql(Request $request)
    {
    
        $brg = DB::SELECT("SELECT * from dataubah where PROS in (69,0,1) and left(NO_BUKTI,2)='KB' order by NO_BUKTI");
					...
					{   
                        ...
                        $btnPrivilege =
                            '
                                    ...
                                    <a class="dropdown-item btn btn-danger" href="import_sql/cetak/' . $row->NO_ID . '">
...
if(!$supd2) $supd2=DB::SELECT("SELECT 0 as HB, 0 AS D1, 0 AS D2, 0 AS D3, 
    ...
    {
        $flag = substr($request->jns, -2); // ambil 2 huruf terakhir, seperti RightStr di Delphi

        try {
            DB::beginTransaction();

            // 1️⃣ Update dataubah
            DB::statement("UPDATE dataubah SET PROS=9 WHERE PROS=1 AND LEFT(NO_BUKTI,2)=?", [$flag]);

            // 2️⃣ Panggil stored procedure
            DB::statement("CALL sim_execmast(?)", [$flag]);
    ...
    public function cetak(Import $import)
    {
        $no_import = $import->NO_BUKTI;

        $file     = 'Pengesahan_Import';
        $PHPJasperXML = new PHPJasperXML();
        $PHPJasperXML->load_xml_file(base_path() . ('/app/reportc01/phpjasperxml/' . $file . '.jrxml'));

        $query = DB::select("CALL sim_repomast('$no_import')");
