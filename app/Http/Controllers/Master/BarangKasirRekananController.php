...
class BarangKasirRekananController extends Controller
{

    public function index()
    {
        $rekan = DB::SELECT("SELECT NAMA FROM rekanan ORDER BY NAMA ASC;");
        $copy = DB::SELECT("SELECT NAMA FROM rekanan ORDER BY NAMA ASC;");

        return view('master_update_barang_kasir_rekanan.index')->with(['rekan' => $rekan])->with(['copy' => $copy]);
    }
    ...
    public function searchBarang(Request $request)
    {
        ...
        $query = DB::table('brg')
            ->select(
                'brg.KD_BRG as id',
                DB::raw("CONCAT(brg.KD_BRG, ' - ', brg.NA_BRG, ' ', COALESCE(brg.KET_UK, '')) as text")
            )
            ->orderBy('brg.KD_BRG', 'ASC');

        if ($search) {
            $query->where(function ($q) use ($search) {
                $q->where('brg.KD_BRG', 'like', '%' . $search . '%')
                    ->orWhere('brg.NA_BRG', 'like', '%' . $search . '%');
            });
        }

        $count = $query->count();
        $barang = $query->offset($offset)->limit($resultCount)->get();
    ...
    public function getUsulanBrgRekanan(Request $request)
    {

        $kd_brg = $request->SUB;
        $rek = $request->rekanan;
        $rekanan = DB::SELECT("SELECT NO_ID FROM rekanan WHERE NAMA='" . $rek . "' ");

        // Ambil NO_ID dari rekanan untuk digunakan di query berikutnya
        $id_rekanan = !empty($rekanan) ? $rekanan[0]->NO_ID : 0;

        $sql = DB::SELECT("SELECT A.NO_ID, A.KD_BRG,
                                        CONCAT(A.NA_BRG,' ' ,A.KET_UK) AS NA_BRG, A.KET_KEM, A.HB,IF(B.KD_BRG IS NULL,0,1) AS JUAL
                                FROM masks A
                                INNER JOIN brg C ON A.KD_BRG=C.KD_BRG
                                LEFT JOIN masks_rekanan B ON B.ID_REKANAN='" . $id_rekanan . "' AND A.KD_BRG=B.KD_BRG
        ...
        return Datatables::of($sql)
            ...
            ->addColumn('action', function ($row) {
                // Return empty string karena action tidak diperlukan untuk usulan barang
                return '';
            })
            ->rawColumns(['action'])
            ->make(true);
    ...
    public function prosesData(Request $request)
    {
        try {
            ...
            $kd_brg_array = $request->kd_brg_array;
            $rekanan = $request->rekanan;

            // Get ID Rekanan
            $rekanan_data = DB::SELECT("SELECT NO_ID FROM rekanan WHERE NAMA='" . $rekanan . "'");

            if (empty($rekanan_data)) {
            ...
            $id_rekanan = $rekanan_data[0]->NO_ID;
            $usrnm = Auth::user()->username;
            $tg_smp = Carbon::now();

            // Process each item
            $processed = 0;
            foreach ($kd_brg_array as $kd_brg) {
                // Check if already exists
                $exists = DB::table('masks_rekanan')
                    ->where('ID_REKANAN', $id_rekanan)
                    ->where('KD_BRG', $kd_brg)
                    ->first();

                if (!$exists) {
                    DB::table('masks_rekanan')->insert([
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Berhasil memproses ' . $processed . ' barang untuk rekanan ' . $rekanan,
                'processed' => $processed
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function copyData(Request $request)
    {
        try {
            ...
            $from_rekanan = $request->from_rekanan;
            $to_rekanan = $request->to_rekanan;

            // Get ID Rekanan Source
            $from_data = DB::SELECT("SELECT NO_ID FROM rekanan WHERE NAMA='" . $from_rekanan . "'");

            // Get ID Rekanan Destination
            $to_data = DB::SELECT("SELECT NO_ID FROM rekanan WHERE NAMA='" . $to_rekanan . "'");
            ...
            $id_from = $from_data[0]->NO_ID;
            $id_to = $to_data[0]->NO_ID;
            $usrnm = Auth::user()->username;
            $tg_smp = Carbon::now();

            // Get data from source
            $source_data = DB::table('masks_rekanan')
                ->where('ID_REKANAN', $id_from)
                ->get();

            if ($source_data->isEmpty()) {
                return response()->json([
                    'status' => 'warning',
                    'message' => 'Tidak ada data untuk di-copy dari rekanan ' . $from_rekanan
                ]);
            }
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Berhasil meng-copy ' . $copied . ' barang dari ' . $from_rekanan . ' ke ' . $to_rekanan,
                'copied' => $copied
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
