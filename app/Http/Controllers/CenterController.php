    ...
    public function pengesahan_brg(Request $request)
    {
        ...
        if (empty($tipe) || empty($cbg) || empty($nafile)) {
            ...
            if ($tipe == 'KMS') {
                ...
                foreach ($request->data['header'] as $rowh) {
                    ...
                    try {
                        ...
                        DB::select("DELETE FROM $dbcenter.brgchd
                        WHERE no_bukti IN (
                            SELECT no_bukti FROM $dbcenter.brgch
                            WHERE NA_FILE='$nafile'
                        )");

                        DB::select("DELETE FROM $dbcenter.brgch
                        WHERE na_file='$nafile'");

                        DB::select("INSERT INTO $dbcenter.brgch
                        (NA_FILE, TYPE, NO_BUKTI, TG_SMP)
                        VALUES ('$nafile', '$dept', '$no_bukti', '$tgl')");
                ...
                foreach ($request->data['detail'] as $rowd) {
                ...
                (no_bukti, sub, kdbar, na_brg, ket_kem, tg_smp, kd_brg, klk, mo, kem_p, supp, id)
                VALUES
                ('$no_bukti', '$sub', '$kdbar', ?, '$ket_kem','$tg_smp',
                 concat('$sub','$kdbar'), '$klk', '$mo', '$kem_p', '$supp', $id)",
                            [$na_brg]
                        );
